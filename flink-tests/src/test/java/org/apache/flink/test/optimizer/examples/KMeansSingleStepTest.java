...
public class KMeansSingleStepTest extends CompilerTestBase {
    ...
    public static Plan kmeans(String[] args) throws Exception {
        ...
        DataSet<Point> points =
                env.readCsvFile(args[0])
                        ...
                        .name(DATAPOINTS)
                        .map(
                                new MapFunction<Tuple2<Double, Double>, Point>() {
                                    @Override
                                    public Point map(Tuple2<Double, Double> value)
                                            throws Exception {
        ...
        DataSet<Centroid> centroids =
                env.readCsvFile(args[1])
                        ...
                        .name(CENTERS)
                        .map(
                                new MapFunction<Tuple3<Integer, Double, Double>, Centroid>() {
                                    @Override
                                    public Centroid map(Tuple3<Integer, Double, Double> value)
                                            throws Exception {
    ...
    public static class Point extends Tuple2<Double, Double> {
        ...
        public double euclideanDistance(Centroid other) {
            return Math.sqrt(
                    (f0 - other.f1.f0) * (f0 - other.f1.f0)
                            + (f1 - other.f1.f1) * (f1 - other.f1.f1));
        }
    }
    ...
    private static final class RecomputeClusterCenter
            implements GroupReduceFunction<
                            Tuple3<Integer, Point, Integer>, Tuple3<Integer, Point, Integer>>,
                    GroupCombineFunction<
                            Tuple3<Integer, Point, Integer>, Tuple3<Integer, Point, Integer>> {

        @Override
        public void reduce(
                Iterable<Tuple3<Integer, Point, Integer>> values,
                Collector<Tuple3<Integer, Point, Integer>> out)
                throws Exception {
            int id = -1;
            double x = 0;
            double y = 0;
