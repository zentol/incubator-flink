...
class StatefulJobWBroadcastStateMigrationITCase
  ...
  def testSavepointWithBroadcast(snapshotSpec:SnapshotSpec, @InjectMiniCluster miniCluster:MiniCluster, @InjectClusterClient miniClusterClient:ClusterClient[_]): Unit = {
    ...
    if (StatefulJobWBroadcastStateMigrationITCase.executionMode == ExecutionMode.CREATE_SNAPSHOT) {
      ...
      executeAndSnapshot(miniCluster, miniClusterClient,
        env,
