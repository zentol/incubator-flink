 ...
 * result in Flink for all TPC-DS queries is strictly matched with TPC-DS answer set.
 */
@SuppressWarnings("UseOfSystemOutOrSystemErr")
public class TpcdsResultComparator {

    private static final List<String> VALIDATE_QUERIES =
            Arrays.asList(
    ...
    private static final HashSet<String> NULL_LITERALS =
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (String queryId : VALIDATE_QUERIES) {
            ...
            if (compareResult(queryId, expectedFile, actualFile)) {
                ...
                System.out.println(
                        String.format(
                                "[INFO] Validation succeeded for file: %s (%d/%d)",
                                expectedFile.getName(), passCnt, VALIDATE_QUERIES.size()));
            } else {
                System.out.println(
                        "[ERROR] Validation failed for file: " + expectedFile.getName() + '\n');
            }
        }
    ...
    private static boolean compareQuery77(String[] expectedLines, String[] actualLines) {
        ...
        if (!comparePairOfLinesPrintingErrors(
                expectedLines[1], expectedLines[2], actualLines[1], actualLines[2], 2, 3)) {
            return false;
        }
    ...
    private static boolean compareQuery79(String[] expectedLines, String[] actualLines) {
        ...
        if (!comparePairOfLinesPrintingErrors(
                expectedLines[17], expectedLines[18], actualLines[17], actualLines[18], 18, 19)) {
            return false;
        }
    ...
    private static boolean compareLinesPrintingErrors(
            String[] expectedLines, String[] actualLines, int from, int num) {
    ...
    private static boolean compareLinesPrintingErrors(
            String[] expectedLines, String[] actualLines, int from) {
    ...
    private static boolean comparePairOfLinesPrintingErrors(
        ...
        final boolean matchesPair =
                compareLine(expected1, actual1) && compareLine(expected2, actual2);
        final boolean matchesCross =
                compareLine(expected2, actual1) && compareLine(expected1, actual2);

        if (!(matchesPair || matchesCross)) {
            System.out.println(
                    String.format(
                            "[ERROR] Lines %d/%d do not match in any pairing."
                                    + "\n - Expected %d: %s\n - Expected %d: %s\n - Actual %d: %s\n - Actual %d: %s",
                            lineNum1, lineNum2, lineNum1, expected1, lineNum2, expected2, lineNum1,
                            actual1, lineNum2, actual2));
    ...
    private static boolean compareLinePrintingErrors(
            String expectedLine, String actualLine, int lineNum) {
        return compareLineInternal(expectedLine, actualLine, lineNum, true);
    }

    private static boolean compareLineInternal(
            String expectedLine, String actualLine, int lineNum, boolean printError) {
