 ...
 * routing feature and restores {@link RouterHandler} which was dropped in tv.cntt:netty-router
 * 2.X.X. Original code:
 * https://github.com/sinetja/netty-router/blob/2.2.0/src/main/java/io/netty/handler/codec/http/router/Router.java
 *
 * <p>Router that contains information about both route matching orders and HTTP request methods.
 *
 ...
 *
 * <p>From the {@link RouteResult} you can extract params embedded in the path and from the query
...
public class Router<T> {
    ...
    /**
     * Returns the fallback target for use when there's no match at {@link #route(HttpMethod,
     * String)}.
     */
    public T notFound() {
        return notFound;
    ...
    public RouteResult<T> route(
            HttpMethod method, String path, Map<String, List<String>> queryParameters) {
        MethodlessRouter<T> router = routers.get(method);
        if (router == null) {
        ...
        if (router != anyMethodRouter) {
            ...
            if (ret != null) {
                return new RouteResult<T>(
                        path, path, ret.pathParams(), queryParameters, ret.target());
            }
        }

        if (notFound != null) {
            return new RouteResult<T>(
                    path, path, Collections.<String, String>emptyMap(), queryParameters, notFound);
        }
