 ...
 * {@link Router}. Original code:
 * https://github.com/sinetja/netty-router/blob/2.2.0/src/main/java/io/netty/handler/codec/http/router/PathPattern.java
 *
 * <p>The pattern can contain constants or placeholders, example: {@code
 * constant1/:placeholder1/constant2/:*}.
 *
 * <p>{@code :*} is a special placeholder to catch the rest of the path (may include slashes). If
 * exists, it must appear at the end of the path.
 *
 * <p>The pattern must not contain query, example: {@code constant1/constant2?foo=bar}.
 *
 * <p>The pattern will be broken to tokens, example: {@code ["constant1", ":variable", "constant2",
 * ":*"]}
 */
