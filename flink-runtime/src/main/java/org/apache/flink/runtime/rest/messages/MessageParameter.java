 ...
 *
 * <p>Parameters are either mandatory or optional, indicating whether the parameter must be resolved
 * for the request.
 *
 * <p>All parameters support symmetric conversion from their actual type and string via {@link
 * #convertFromString(String)} and {@link #convertToString(Object)}. The conversion from {@code X}
 * to string is required on the client to assemble the URL, whereas the conversion from string to
 * {@code X} is required on the server to provide properly typed parameters to the handlers.
 *
