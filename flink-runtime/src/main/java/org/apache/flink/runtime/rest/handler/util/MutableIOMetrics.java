 ...
 * metrics.
 *
 * <p>For finished jobs these metrics are stored in the {@link ExecutionGraph} as another {@link
 * IOMetrics}. For running jobs these metrics are retrieved using the {@link MetricFetcher}.
 *
 * <p>This class provides a common interface to handle both cases, reducing complexity in various
 * handlers (like the {@link JobVertexDetailsHandler}).
 */
public class MutableIOMetrics extends IOMetrics {
    ...
    /**
     * Adds the IO metrics for the given attempt to this object. If the {@link AccessExecution} is
     * in a terminal state the contained {@link IOMetrics} object is added. Otherwise the given
        ...
        if (attempt.getState().isTerminal()) {
            ...
            if (fetcher != null) {
                ...
                if (metrics != null) {
                     ...
                     * between 0 as a value and a missing value. In case a metric is missing for a
                     * parallel instance of a task, we set the complete flag as false.
                     */
