...
/**
 * Implementation of the {@link TypePairComparator} interface for Pact Records. The equality is
 * established on a set of key fields. The indices of the key fields may be different on the
 * reference and candidate side.
 */
public class RecordPairComparator extends TypePairComparator<Record, Record> {

    private final int[] keyFields1,
            keyFields2; // arrays with the positions of the keys in the records
    @SuppressWarnings("rawtypes")
    private final Value[] keyHolders1, keyHolders2; // arrays with mutable objects for the key types

    public RecordPairComparator(
            int[] keyFieldsReference, int[] keyFieldsCandidate, Class<? extends Value>[] keyTypes) {
        if (keyFieldsReference.length != keyFieldsCandidate.length
                || keyFieldsCandidate.length != keyTypes.length) {
    ...
    public int compareToReference(Record candidate) {
        for (int i = 0; i < this.keyFields2.length; i++) {
            final Comparable k =
                    (Comparable) candidate.getField(this.keyFields2[i], this.keyHolders2[i]);
            if (k == null) {
                throw new NullKeyFieldException(this.keyFields2[i]);
            } else {
