 ...
 *
 * <p>The 'extractor' {@link Function} generates a 'container', a jackson-compatible object
 * containing the data that the routine bases it's compatibility-evaluation on. The 'assertion'
 * {@link BiConsumer} accepts a pair of containers and asserts the compatibility. Incompatibilities
 * are signaled by throwing an {@link AssertionError}. This implies that the method body will
 * typically contain jUnit assertions.
 */
final class CompatibilityRoutine<C> {
    ...
    CompatibilityCheckResult checkCompatibility(final Optional<C> old, final Optional<C> cur) {
        Preconditions.checkArgument(
                ...
                "Implementation error: Compatibility check container for routine %s for both old and new version is null.",
                key);
        ...
        if (!cur.isPresent()) {
            ...
            return CompatibilityCheckResult.incompatible(
                    new AssertionError(
                            String.format(
                                    "Compatibility check container for routine %s not found in current version.",
                                    key)));
        }
        ...
        try {
            ...
            final AssertionError backwardIncompatibilityCause =
                    new AssertionError(key + ": " + e.getMessage());
