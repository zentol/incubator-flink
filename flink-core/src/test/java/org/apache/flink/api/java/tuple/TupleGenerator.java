...
class TupleGenerator {
    ...
    private static void writeTupleClass(PrintWriter w, int numFields) {
        ...
        w.println(
                " * A tuple with "
                        + numFields
                        + " fields. Tuples are strongly typed; each field may be of a separate type.");
        w.println(
        ...
        w.println(
                " *       type. (That is, if you have a \"class Foo extends "
                        + className
                        + "\", then don't use instances of");
        w.println(
                " *       Foo in a DataStream&lt;"
                        + className
                        + "&gt; / DataSet&lt;"
