 ...
 *
 * <p>The configuration captures the order of Kryo serializer registrations, so that new Kryo
 * serializers can determine how to reconfigure their registration order to retain backwards
 * compatibility.
 *
...
public abstract class KryoRegistrationSerializerConfigSnapshot<T>
    ...
    private static class KryoRegistrationSerializationProxy<RC> implements IOReadableWritable {
        ...
        public void read(DataInputView in) throws IOException {
            ...
            try {
                registeredClass =
                        (Class<RC>) Class.forName(registeredClassname, true, userCodeClassLoader);
                    ...
                    try (final DataInputViewStream inViewWrapper = new DataInputViewStream(in)) {
                        ...
                        serializerInstance =
                                new ExecutionConfig.SerializableSerializer<>(
                                        new DummyKryoSerializerClass<RC>());
    ...
    /**
     * Placeholder dummy for a previously registered class that can no longer be found in classpath
     * on restore.
     */
    public static class DummyRegisteredClass implements Serializable {}

    /**
     * Placeholder dummy for a previously registered Kryo serializer that is no longer valid or in
     * classpath on restore.
     */
    ...
    public boolean equals(Object obj) {
        return super.equals(obj)
                && kryoRegistrations.equals(
                        ((KryoSerializer.KryoSerializerConfigSnapshot) obj).getKryoRegistrations());
    }
