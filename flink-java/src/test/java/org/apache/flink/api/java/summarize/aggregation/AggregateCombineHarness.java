...
/**
 * This harness uses multiple aggregators and variously aggregates and combines against a list of
 * values while calling a compareResults() method.
 *
 * <p>This method breaks the rule of "testing only one thing" by aggregating and combining a bunch
 * of different ways but can help uncover various kinds of bugs that can show up in aggregators.
 *
...
public abstract class AggregateCombineHarness<T, R, A extends Aggregator<T, R>> {
    ...
    public final R summarize(T... values) {
        if (values.length == 0) {
            // when there is nothing to aggregate just combine two empty aggregators and get the
            // result.
            ...
            for (int i = 0; i < values.length; i++) {
                ...
                // It shouldn't matter which aggregator is used because they are combined at the end
                // so
                // we're looping through all points of the data and making sure it doesn't make a
                // difference.
                ...
                if (previousResult != null) {
                    // validate that variously aggregating then combining doesn't give different
                    // results
