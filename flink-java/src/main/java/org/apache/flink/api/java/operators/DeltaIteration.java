...
public class DeltaIteration<ST, WT> {
     ...
     * @param newWorkset The new workset (feedback data set) that will be fed back to the next
     *     iteration.
     ...
     * }</pre>
     *
     * <p>The <tt>solutionSetData</tt> would be the data set returned by {@code
     * iteration.getInitialSolutionSet();}.
     *
    ...
    /**
     * Gets the solution set of the delta iteration. The solution set represents the state that is
    ...
    /**
     * Gets the working set of the delta iteration. The working set is constructed by the previous
     * iteration.
     *
     ...
     *
     * @return The iteration's parallelism, or {@link ExecutionConfig#PARALLELISM_DEFAULT} if not
     *     set.
     */
    ...
    /**
     * Sets the minimum and preferred resources for the iteration. This overrides the default
     ...
     */
    private DeltaIteration<ST, WT> setResources(
    ...
    /**
     * Gets the minimum resources from this iteration. If no minimum resources have been set, it
    ...
    /**
     * Gets the preferred resources from this iteration. If no preferred resources have been set, it
     * returns the default empty resource.
     *
        ...
        public void checkJoinKeyFields(int[] keyFields) {
            ...
            if (!Arrays.equals(ssKeys, keyFields)) {
                throw new InvalidProgramException(
                        "The solution can only be joined/co-grouped with the same keys as the elements are identified with (here: "
                                + Arrays.toString(ssKeys)
                                + ").");
            }
        }
    }
