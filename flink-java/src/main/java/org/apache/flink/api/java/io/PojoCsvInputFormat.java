...
public class PojoCsvInputFormat<OUT> extends CsvInputFormat<OUT> {
    ...
    public PojoCsvInputFormat(Path filePath, PojoTypeInfo<OUT> pojoTypeInfo, String[] fieldNames) {
        this(
                ...
                pojoTypeInfo,
                fieldNames,
                createDefaultMask(pojoTypeInfo.getArity()));
    }
    ...
    public PojoCsvInputFormat(
            Path filePath, PojoTypeInfo<OUT> pojoTypeInfo, int[] includedFieldsMask) {
        this(
                filePath,
                DEFAULT_LINE_DELIMITER,
                DEFAULT_FIELD_DELIMITER,
    ...
    public PojoCsvInputFormat(
            Path filePath, PojoTypeInfo<OUT> pojoTypeInfo, boolean[] includedFieldsMask) {
        this(
                filePath,
                DEFAULT_LINE_DELIMITER,
                DEFAULT_FIELD_DELIMITER,
    ...
    private void configure(
        ...
        for (int i = 0; i < fieldNames.length; i++) {
            try {
                classes[i] =
                        pojoTypeInfo
                                .getTypeAt(pojoTypeInfo.getFieldIndex(fieldNames[i]))
                                .getTypeClass();
    ...
    private void setOrderOfPOJOFields(String[] fieldNames) {
        ...
        Preconditions.checkArgument(
                includedCount == fieldNames.length,
                includedCount
                        + " CSV fields and "
                        + fieldNames.length
                        + " POJO fields selected. The number of selected CSV and POJO fields must be equal.");

        for (String field : fieldNames) {
            Preconditions.checkNotNull(field, "The field name cannot be null.");
            Preconditions.checkArgument(
                    pojoTypeInfo.getFieldIndex(field) != -1,
                    "Field \""
                            + field
                            + "\" is not a member of POJO class "
                            + pojoTypeClass.getName());
        }
    ...
    public OUT fillRecord(OUT reuse, Object[] parsedValues) {
        for (int i = 0; i < parsedValues.length; i++) {
            try {
                ...
                throw new RuntimeException(
                        "Parsed value could not be set in POJO field \"" + pojoFieldNames[i] + "\"",
                        e);
            }
        }
        return reuse;
