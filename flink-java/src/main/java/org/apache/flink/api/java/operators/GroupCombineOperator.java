 ...
 * which reduces all data locally in their partitions. The combine part can return an arbitrary data
 * type. This is useful to pre-combine values into an intermediate representation before applying a
 * proper reduce operation.
 *
...
public class GroupCombineOperator<IN, OUT>
    ...
    protected GroupCombineOperatorBase<?, OUT, ?> translateToDataFlow(Operator<IN> input) {
        ...
        if (grouper.getKeys() instanceof SelectorFunctionKeys) {
            ...
            if (grouper instanceof SortedGrouping) {
                ...
                PlanUnwrappingGroupCombineOperator<IN, OUT, ?> po =
    ...
    private static <IN, OUT, K>
            PlanUnwrappingGroupCombineOperator<IN, OUT, K> translateSelectorFunctionReducer(
        ...
        PlanUnwrappingGroupCombineOperator<IN, OUT, K> reducer =
                new PlanUnwrappingGroupCombineOperator<>(
    ...
    private static <IN, OUT, K1, K2>
        ...
        PlanUnwrappingSortedGroupCombineOperator<IN, OUT, K1, K2> reducer =
                new PlanUnwrappingSortedGroupCombineOperator<>(
                        function, groupingKey, sortingKey, name, outputType, typeInfoWithKey);
