...
public class RequiredParameters {
        ...
        for (Option o : data.values()) {
            if (newParameters.containsKey(o.getName())) {
                if (Objects.equals(newParameters.get(o.getName()), ParameterTool.NO_VALUE_KEY)) {
                    // the parameter has been passed, but no value, check if there is a default
                    // value
                    checkAndApplyDefaultValue(o, newParameters);
                } else {
                    // a value has been passed in the parameterTool, now check if it adheres to all
                    // constraints
                ...
                // check if there is a default name or a value passed for a possibly defined
                // alternative name.
    ...
    // check if the given parameter has a default value and add it to the passed map if that is the
    // case
    // else throw an exception
    private void checkAndApplyDefaultValue(Option o, Map<String, String> data)
            throws RequiredParametersException {
        if (hasNoDefaultValueAndNoValuePassedOnAlternativeName(o, data)) {
            throw new RequiredParametersException(
                    "No default value for undefined parameter " + o.getName());
        }
    }
    ...
    private void checkIsCastableToDefinedType(Option o, Map<String, String> data)
            throws RequiredParametersException {
    ...
    private void checkChoices(Option o, Map<String, String> data)
            throws RequiredParametersException {
        if (o.getChoices().size() > 0 && !o.getChoices().contains(data.get(o.getName()))) {
            throw new RequiredParametersException(
                    "Value "
                            + data.get(o.getName())
                            + " is not in the list of valid choices for key "
                            + o.getName());
        }
    }
    ...
    private boolean hasNoDefaultValueAndNoValuePassedOnAlternativeName(
    ...
    private void checkAmbiguousValues(Option o, Map<String, String> data)
            throws RequiredParametersException {
        if (data.containsKey(o.getAlt())
                && !Objects.equals(data.get(o.getAlt()), ParameterTool.NO_VALUE_KEY)) {
            throw new RequiredParametersException(
     ...
     *
     * <p>The format of the help text will be: Required Parameters: \t -:shortName:, --:name: \t
     * :helpText: \t default: :defaultValue: \t choices: :choices: \n
     *
    ...
    /**
     * Build a help text for the defined parameters and list the missing arguments at the end of the
     * text.
     *
     * <p>The format of the help text will be: Required Parameters: \t -:shortName:, --:name: \t
     * :helpText: \t default: :defaultValue: \t choices: :choices: \n
     *
     * <p>Missing parameters: \t param1 param2 ... paramN
     *
    ...
    /**
     * for the given option create a line for the help text. The line looks like: \t -:shortName:,
     * --:name: \t :helpText: \t default: :defaultValue: \t choices: :choices:
     */
