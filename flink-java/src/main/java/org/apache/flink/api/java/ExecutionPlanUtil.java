...
public class ExecutionPlanUtil {
    ...
    private static ExecutionPlanJSONGenerator getJSONGenerator() {
        Class<? extends ExecutionPlanJSONGenerator> planGeneratorClass =
                loadJSONGeneratorClass(PLAN_GENERATOR_CLASS_NAME);

        try {
            return planGeneratorClass.getConstructor().newInstance();
        } catch (Throwable t) {
            throw new RuntimeException(
                    "An error occurred while loading the plan generator ("
    ...
    private static Class<? extends ExecutionPlanJSONGenerator> loadJSONGeneratorClass(
            String className) {
        try {
            ...
            throw new RuntimeException(
                    "Could not load the plan generator class ("
                            + className
                            + "). Do you have the 'flink-optimizer' project in your dependencies?");
        } catch (Throwable t) {
            throw new RuntimeException(
                    "An error occurred while loading the plan generator (" + className + ").", t);
        }
    }
