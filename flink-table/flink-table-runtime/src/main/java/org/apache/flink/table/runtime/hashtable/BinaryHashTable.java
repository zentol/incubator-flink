...
public class BinaryHashTable extends BaseHybridHashTable {
    ...
    public BinaryHashTable(
        ...
        this.binaryBuildSideSerializer =
                new BinaryRowDataSerializer(buildSideSerializer.getArity());
        ...
        this.binaryProbeSideSerializer =
                new BinaryRowDataSerializer(originProbeSideSerializer.getArity());
        ...
        checkArgument(
                bucketsPerSegment != 0,
                "Hash Table requires buffers of at least "
                        + BinaryHashBucketArea.BUCKET_SIZE
                        + " bytes.");
    ...
    private boolean processBuildIter() throws IOException {
        ...
        this.buildIterator =
                new BuildSideIterator(
                        this.binaryBuildSideSerializer,
                        reuseBuildRow,
                        this.partitionsBeingBuilt,
                        probedSet,
                        type.equals(HashJoinType.BUILD_LEFT_SEMI));
            ...
            this.buildIterator =
                    new WrappedRowIterator<>(
                            new BinaryRowChannelInputViewIterator(
                                    currentSpilledBuildSide, this.binaryBuildSideSerializer),
    ...
    private void buildTableFromSpilledPartition(final BinaryHashPartition p) throws IOException {
        ...
        long numBuckets =
                p.getBuildSideRecordCount() / BinaryHashBucketArea.NUM_ENTRIES_PER_BUCKET + 1;
        ...
        int maxBucketAreaBuffers =
                Math.max((int) (2 * (numBuckets / (this.bucketsPerSegmentMask + 1))), 1);
        ...
        if (totalBuffersNeeded < totalBuffersAvailable) {
            ...
            BinaryHashBucketArea area =
                    new BinaryHashBucketArea(
                            this, (int) p.getBuildSideRecordCount(), maxBucketAreaBuffers, false);
            final BinaryHashPartition newPart =
                    new BinaryHashPartition(
                            area,
                            this.binaryBuildSideSerializer,
                            this.binaryProbeSideSerializer,
                            0,
            ...
            while (pIter.advanceNext()) {
                final int hashCode =
                        hash(
                                buildSideProjection.apply(pIter.getRow()).hashCode(),
                                nextRecursionLevel);
            ...
            while ((rec = inIter.next(rec)) != null) {
                final int hashCode =
                        hash(this.buildSideProjection.apply(rec).hashCode(), nextRecursionLevel);
    ...
    private void insertIntoTable(final BinaryRowData record, final int hashCode)
            throws IOException {
    ...
    private void createPartitions(int numPartitions, int recursionLevel) {
        ...
        for (int i = 0; i < numPartitions; i++) {
            BinaryHashBucketArea area =
                    new BinaryHashBucketArea(this, numRecordPerPartition, maxBuffer);
    ...
    boolean applyCondition(BinaryRowData candidate) {
        ...
        boolean equal =
                buildKey.getSizeInBytes() == probeKey.getSizeInBytes()
        ...
        if (!nullSafe) {
            equal =
                    equal
                            && !(filterAllNulls
                                    ? buildKey.anyNull()
                                    : buildKey.anyNull(nullFilterKeys));
        }
        return condFunc == null
                ? equal
                : equal
                        && (reverseJoin
                                ? condFunc.apply(probeRow, candidate)
