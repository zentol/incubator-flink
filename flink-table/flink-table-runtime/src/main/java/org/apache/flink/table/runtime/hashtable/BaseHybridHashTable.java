...
public abstract class BaseHybridHashTable implements MemorySegmentPool {
     ...
     * behind buffers, but regular buffers that only have not yet returned. This is part of an
     * optimization that the spilling code needs not wait until the partition is completely spilled
     * before proceeding.
     */
    ...
    public BaseHybridHashTable(
        ...
        LOG.info(
                String.format(
                        "Initialize hash table with %d memory segments, each size [%d], the memory %d MB.",
                        totalNumBuffers,
                        segmentSize,
                        (long) totalNumBuffers * segmentSize / 1024 / 1024));
    }

    /**
     * Bucket area need at-least one and data need at-least one. In the initialization phase, we can
     * use (totalNumBuffers - numWriteBehindBuffers) Segments. However, in the
     * buildTableFromSpilledPartition phase, only (totalNumBuffers - numWriteBehindBuffers - 2) can
     * be used because two Buffers are needed to read the data.
     */
    ...
    public int freePages() {
        throw new UnsupportedOperationException(
                "Contains spill memories, it is hard to estimate free pages.");
    }
    ...
    public void free() {
        if (this.closed.get()) {
            ...
            throw new IllegalStateException(
                    "Cannot release memory until BinaryHashTable is closed!");
        }
    }
    ...
    protected List<MemorySegment> readAllBuffers(FileIOChannel.ID id, int blockCount)
        ...
        BlockChannelReader<MemorySegment> reader =
                FileChannelUtil.createBlockChannelReader(
                        ioManager,
                        id,
                        retSegments,
                        compressionEnable,
    ...
    protected HeaderlessChannelReaderInputView createInputView(
            FileIOChannel.ID id, int blockCount, int lastSegmentLimit) throws IOException {
        BlockChannelReader<MemorySegment> inReader =
                FileChannelUtil.createBlockChannelReader(
                        ioManager,
                        id,
