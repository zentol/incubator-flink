...
/**
 * The operator for OVER window don't need cache data. Then this operator can calculate the
 * accumulator by the current row.
 *
 * <p>Some over windows do not need to buffer data, such as {@code rows between unbounded preceding
 * and 0}, rank, etc. We introduce {@link NonBufferOverWindowOperator} to reduce the overhead of
 * data copy in buffer.
 *
...
public class NonBufferOverWindowOperator extends TableStreamOperator<RowData>
    ...
    public void processElement(StreamRecord<RowData> element) throws Exception {
        ...
        boolean changePartition =
                lastInput == null || partitionComparator.compare(lastInput, input) != 0;
