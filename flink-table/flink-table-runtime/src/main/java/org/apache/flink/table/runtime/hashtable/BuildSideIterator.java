...
public class BuildSideIterator implements RowIterator<BinaryRowData> {
    ...
    public static class BucketIterator implements RowIterator<BinaryRowData> {
        ...
        private void setBucket(
                MemorySegment bucket, MemorySegment[] overflowSegments, int bucketInSegmentOffset) {
            ...
            this.countInBucket =
                    bucket.getShort(
                            bucketInSegmentOffset + BinaryHashBucketArea.HEADER_COUNT_OFFSET);
            this.numInBucket = 0;
            // reset probedSet with probedFlags offset in this bucket.
            this.probedSet.setMemorySegment(
                    bucketSegment,
                    this.bucketInSegmentOffset + BinaryHashBucketArea.PROBED_FLAG_OFFSET);
        }
            ...
            while (countInBucket != 0) {
                ...
                while (this.numInBucket < this.countInBucket) {
                    ...
                    if (matchedOrUnmatched == probed) {
                        try {
                            this.area.partition.setReadPosition(
                                    bucketSegment.getInt(pointerOffset));
                ...
                this.countInBucket =
                        this.bucketSegment.getShort(
                                this.bucketInSegmentOffset
                                        + BinaryHashBucketArea.HEADER_COUNT_OFFSET);
                this.numInBucket = 0;
                // reset probedSet with probedFlags offset in this bucket.
                this.probedSet.setMemorySegment(
                        bucketSegment,
