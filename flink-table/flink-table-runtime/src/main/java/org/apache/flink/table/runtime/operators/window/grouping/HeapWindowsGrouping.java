...
/**
 * A jvm heap implementation of {@link WindowsGrouping}, which uses a linked list to buffer all the
...
public class HeapWindowsGrouping extends WindowsGrouping {
    ...
    public HeapWindowsGrouping(
            int maxSizeLimit, long windowSize, long slideSize, int timeIndex, boolean isDate) {
    ...
    protected void addIntoBuffer(BinaryRowData input) throws IOException {
        if (buffer.size() >= maxSizeLimit) {
            throw new IOException(
                    "HeapWindowsGrouping out of memory, element size limit " + maxSizeLimit);
        }
        buffer.add(input);
    }
