...
/**
 * The {@link BinaryExternalSorter} is a full fledged sorter for binary format. It implements a
 ...
 * using the {@link MemoryManager} interface. Thus the component will not exceed the provided memory
 * limits.
 */
public class BinaryExternalSorter implements Sorter<BinaryRowData> {
    ...
    public BinaryExternalSorter(
        ...
        if (reservedMemorySize < SORTER_MIN_NUM_SORT_MEM) {
            throw new IllegalArgumentException(
                    "Too little memory provided to sorter to perform task. "
                            ...
                            + SORTER_MIN_NUM_SORT_MEM
                            + " bytes. Current memory size is "
                            + reservedMemorySize
                            + " bytes.");
        }
        ...
        LOG.info(
                "BinaryExternalSorter with initial memory segments {}, "
        ...
        for (int i = 0; i < numSortBuffers; i++) {
            ...
            int sortSegments =
                    Math.min(
                            i == numSortBuffers - 1 ? Integer.MAX_VALUE : numSegmentsPerSortBuffer,
                            totalBuffers);
            totalBuffers -= sortSegments;
            LazyMemorySegmentPool pool =
                    new LazyMemorySegmentPool(owner, memoryManager, sortSegments);
        ...
        this.mergeThread =
                getMergingThread(exceptionHandler, circularQueues, maxNumFileHandles, merger);
     ...
     * is called.
     */
    private abstract static class ThreadBase extends Thread
    ...
    private class SpillingThread extends ThreadBase {
        ...
        public void go() throws IOException {
            ...
            // ------------------- In-Memory Merge ------------------------
            if (cacheOnly) {
                List<MutableObjectIterator<BinaryRowData>> iterators =
                        new ArrayList<>(cache.size());
            ...
            while (isRunning()) {
                try {
                    ...
                    if (isRunning()) {
                        LOG.error(
                                "Spilling thread was interrupted (without being shut down) while grabbing a buffer. "
                                        + "Retrying to grab buffer...");
                        continue;
                    } else {
                        return;
                ...
                if (element.buffer.getOccupancy() > 0) {
                    ...
                    try {
                        ...
                        LOG.info(
                                "here spill the {}th sort buffer data with {} bytes and {} compressed bytes",
                                numSpillFiles,
                                spillInBytes,
                    ...
                    this.queues.merge.add(
                            new ChannelWithMeta(channel, blockCount, bytesInLastBuffer));
                }
        ...
        private void releaseEmptyBuffers() {
            while (!this.queues.empty.isEmpty()) {
                try {
                    ...
                    if (isRunning()) {
                        LOG.error(
                                "Spilling thread was interrupted (without being shut down) while collecting empty "
    ...
    /**
     * The thread that merges the intermediate spill files and the merged files until sufficiently
     * few channels remain to perform the final streamed merge.
     */
