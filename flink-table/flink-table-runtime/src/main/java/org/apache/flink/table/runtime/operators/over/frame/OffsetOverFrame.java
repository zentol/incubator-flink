...
public class OffsetOverFrame implements OverWindowFrame {
    ...
    public void open(ExecutionContext ctx) throws Exception {
        processor =
                aggsHandleFunction.newInstance(ctx.getRuntimeContext().getUserCodeClassLoader());
    ...
    public RowData process(int index, RowData current) throws Exception {
        if (calcOffsetFunc != null) {
            ...
            if (realIndex >= 0 && realIndex < currentBufferLength) {
                ResettableExternalBuffer.BufferIterator tempIterator =
                        externalBuffer.newIterator((int) realIndex);
