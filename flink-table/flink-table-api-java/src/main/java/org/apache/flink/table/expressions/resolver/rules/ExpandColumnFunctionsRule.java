...
/**
 * Replaces column functions with all available {@link
 * org.apache.flink.table.expressions.UnresolvedReferenceExpression}s from underlying inputs.
 */
@Internal
                    ...
                    for (int i = 1; i < args.size(); ++i) {
                        if (!(args.get(i) instanceof ValueLiteralExpression)) {
                            ...
                            throw new ValidationException(
                                    String.format(
                                            "Invalid AS, parameters are: [%s].", errorMessage));
                        }
                    }
                }
            }
