 ...
 * point in a number (=scale). {@code p} must have a value between 1 and 38 (both inclusive). {@code
 * s} must have a value between 0 and {@code p} (both inclusive). The default value for {@code p} is
 * 10. The default value for {@code s} is 0. {@code NUMERIC(p, s)} and {@code DEC(p, s)} are
