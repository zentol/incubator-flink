 ...
 *
 * <p>When translating relational plan to execution plan in the {@link Planner}, the generated
 * {@link Transformation}s will be added into StreamExecutionEnvironment's buffer, and they will be
 * cleared only when {@link StreamExecutionEnvironment#execute()} method is called. Each {@link
 * TableEnvironment} instance holds an immutable StreamExecutionEnvironment instance. If there are
 ...
 * incorrect.
 *
 * <p>This dummy StreamExecutionEnvironment is only used for buffering the transformations generated
 * in the planner. A new dummy StreamExecutionEnvironment instance should be created for each
 * translation, and this could avoid dirty the transformation buffer of the real
 * StreamExecutionEnvironment instance.
 *
 * <p>All set methods (e.g. `setXX`, `enableXX`, `disableXX`, etc) are disabled to prohibit changing
 * configuration, all get methods (e.g. `getXX`, `isXX`, etc) will be delegated to the real
 * StreamExecutionEnvironment. `execute`, `getStreamGraph`, `getExecutionPlan` methods are also
 * disabled, while `addOperator` method is enabled to allow the planner to add the generated
 * transformations to the dummy StreamExecutionEnvironment.
 *
 * <p>This class could be removed once the {@link StreamTableSource} interface and {@link
 * StreamTableSink} interface are reworked.
 *
 * <p>NOTE: Please remove {@code com.esotericsoftware.kryo} item in the whitelist of
 * checkCodeDependencies() method in {@code test_table_shaded_dependencies.sh} end-to-end test when
