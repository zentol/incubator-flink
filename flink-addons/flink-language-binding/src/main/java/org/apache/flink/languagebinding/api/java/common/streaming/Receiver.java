...
public class Receiver implements Serializable {
	...
	public Object getRecord() throws IOException {
		...
		if (type == TYPE_TUPLE) {
			int size = fileBuffer.get();
			Tuple tuple = createTuple(size);
			for (int x = 0; x < size; x++) {
				tuple.setField(receiveField(fileBuffer.get()), x);
			}
			return tuple;
		} else {
			return receiveField(type);
		}
	}
	 ...
	 * @throws IOException
	 */
	private Object receiveField(byte type) throws IOException {
		switch (type) {
			case TYPE_TUPLE:
	...
	/**
	 * Returns a record from the buffer, for which similar typed fields are type-normalized. Note: This method cannot be
	 * replaced with specific methods like readInt() or similar. The PlanBinder requires a method that can return any
	 * kind of object.
	 *
	 * @return read record
	 * @throws IOException
	 */
	public Object getNormalizedRecord() throws IOException {
		if (fileBuffer.position() == 0) {
			loadBuffer();
		}
		byte type = fileBuffer.get();
		if (type == TYPE_TUPLE) {
			...
			Tuple tuple = createTuple(size);
			for (int x = 0; x < size; x++) {
				tuple.setField(receiveNormalizedField(fileBuffer.get()), x);
			}
			return tuple;
		} else {
			return receiveNormalizedField(type);
		}
	}
	 ...
	 * @throws IOException
	 */
	private Object receiveNormalizedField(byte type) throws IOException {
		switch (type) {
			case TYPE_TUPLE:
				int tupleSize = fileBuffer.get();
				Tuple tuple = createTuple(tupleSize);
			...
			case TYPE_BYTE:
				return fileBuffer.get();
			case TYPE_SHORT:
				return new Short(fileBuffer.getShort()).intValue();
			case TYPE_INTEGER:
				return fileBuffer.getInt();
			case TYPE_LONG:
