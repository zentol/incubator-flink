...
public class SkipListUtils {
    ...
    /**
     * Key space schema. - key meta -- int: level & status -- byte 0: level of node in skip list --
     * byte 1: node status -- byte 2: preserve -- byte 3: preserve -- int: length of key -- long:
     * pointer to the newest value -- long: pointer to next node on level 0 -- long[]: array of
     * pointers to next node on different levels excluding level 0 -- long[]: array of pointers to
     * previous node on different levels excluding level 0 - byte[]: data of key
     */
    ...
    /**
     * Pre-compute the offset of index for different levels to dismiss the duplicated computation at
     * runtime.
     */
    private static final int[] INDEX_NEXT_OFFSET_BY_LEVEL_ARRAY = new int[MAX_LEVEL + 1];

    /**
     * Pre-compute the length of key meta for different levels to dismiss the duplicated computation
     * at runtime.
     */
     ...
     */
    public static void putLevelAndNodeStatus(
            MemorySegment memorySegment, int offset, int level, NodeStatus status) {
     ...
     */
    public static void putNextKeyPointer(
            MemorySegment memorySegment, int offset, long nextKeyPointer) {
    ...
    /**
     * Value space schema. - value meta -- int: version of this value to support copy on write --
     * long: pointer to the key space -- long: pointer to next older value -- int: length of data -
     * byte[] data of value
     */
     ...
     */
    public static void putNextValuePointer(
