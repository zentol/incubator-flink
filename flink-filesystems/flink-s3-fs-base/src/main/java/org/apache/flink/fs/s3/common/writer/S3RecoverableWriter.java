 ...
 *
 * <p>This implementation makes heavy use of MultiPart Uploads in S3 to persist intermediate data as
 * soon as possible.
 *
 * <p>This class partially reuses utility classes and implementations from the Hadoop project,
 * specifically around configuring S3 requests and handling retries.
 */
@PublicEvolving
public class S3RecoverableWriter implements RecoverableWriter {
    ...
    @Override
    public S3RecoverableFsDataOutputStream recover(ResumeRecoverable recoverable)
            throws IOException {
        final S3Recoverable s3recoverable = castToS3Recoverable(recoverable);
        ...
        return S3RecoverableFsDataOutputStream.recoverStream(
                upload, tempFileCreator, userDefinedMinPartSize, s3recoverable.numBytesInParts());
    }
    ...
    public static S3RecoverableWriter writer(
        ...
        return new S3RecoverableWriter(
                s3AccessHelper, uploadFactory, tempFileCreator, userDefinedMinPartSize);
    }
}
