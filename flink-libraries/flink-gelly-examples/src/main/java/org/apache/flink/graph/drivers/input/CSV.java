...
/**
 * Read a {@link Graph} from a CSV file using {@link IntValue}, {@link LongValue}, or {@link
 * StringValue} keys.
 *
 * @param <K> key type
 */
public class CSV<K extends Comparable<K>> extends InputBase<K, NullValue, NullValue> {
    ...
    private ChoiceParameter type =
            new ChoiceParameter(this, "type").setDefaultValue(INTEGER).addChoices(LONG, STRING);

    private StringParameter inputFilename = new StringParameter(this, "input_filename");

    private StringParameter commentPrefix =
            new StringParameter(this, "comment_prefix").setDefaultValue("#");

    private StringParameter lineDelimiter =
            new StringParameter(this, "input_line_delimiter")
                    .setDefaultValue(CsvInputFormat.DEFAULT_LINE_DELIMITER);

    private StringParameter fieldDelimiter =
            new StringParameter(this, "input_field_delimiter")
    ...
    public String getIdentity() {
        return WordUtils.capitalize(getName())
                + WordUtils.capitalize(type.getValue())
                + " ("
                + inputFilename
                + ")";
    }
