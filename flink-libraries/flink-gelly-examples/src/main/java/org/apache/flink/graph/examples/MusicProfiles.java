...
/**
 * This example demonstrates how to mix the DataSet Flink API with the Gelly API. The input is a set
 * &lt;userId - songId - playCount&gt; triplets and a set of bad records, i.e. song ids that should
 * not be trusted. Initially, we use the DataSet API to filter out the bad records. Then, we use
 * Gelly to create a user -&gt; song weighted bipartite graph and compute the top song (most
 * listened) per user. Then, we use the DataSet API again, to create a user-user similarity graph,
 * based on common songs, where users that are listeners of the same song are connected. A
 * user-defined threshold on the playcount value defines when a user is considered to be a listener
 * of a song. Finally, we use the graph API to run the label propagation community detection
 * algorithm on the similarity graph.
 *
 * <p>The triplets input is expected to be given as one triplet per line, in the following format:
 * "&lt;userID&gt;\t&lt;songID&gt;\t&lt;playcount&gt;".
 *
 * <p>The mismatches input file is expected to contain one mismatch record per line, in the
 * following format: "ERROR: &lt;songID trackID&gt; song_title"
 *
 * <p>If no arguments are provided, the example runs with default data from {@link
 * MusicProfilesData}.
 */
...
public class MusicProfiles implements ProgramDescription {
    ...
    public static void main(String[] args) throws Exception {
        ...
        /**
         * Create a user -> song weighted bipartite graph where the edge weights correspond to play
         * counts
         */
        ...
        /**
         * Create a user-user similarity graph, based on common songs, i.e. two users that listen to
         * the same song are connected. For each song, we create an edge between each pair of its
         * in-neighbors.
         */
        ...
        // Initialize each vertex with a unique numeric label and run the label propagation
        // algorithm
