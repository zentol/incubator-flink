...
/**
 * This example shows how to use Gelly's {@link Graph#getTriplets()} and {@link
 * Graph#joinWithEdges(DataSet, EdgeJoinFunction)} methods.
 *
 * <p>Given a directed, unweighted graph, with vertex values representing points in a plain, return
 * a weighted graph where the edge weights are equal to the Euclidean distance between the src and
 * the trg vertex values.
 *
 * <p>Input files are plain text files and must be formatted as follows:
 *
 * <ul>
 *   <li>Vertices are represented by their vertexIds and vertex values and are separated by
 *       newlines, the value being formed of two doubles separated by a comma. For example: <code>
 *       1,1.0,1.0\n2,2.0,2.0\n3,3.0,3.0\n</code> defines a data set of three vertices
 *   <li>Edges are represented by pairs of srcVertexId, trgVertexId separated by commas. Edges
 *       themselves are separated by newlines. For example: <code>1,2\n1,3\n</code> defines two
 *       edges 1-2 and 1-3.
 * </ul>
 *
 * <p>Usage <code>EuclideanGraphWeighing &lt;vertex path&gt; &lt;edge path&gt; &lt;result path&gt;
 * </code><br>
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataSet<Tuple3<Long, Long, Double>> edgesWithEuclideanWeight =
                graph.getTriplets()
                        .map(
                                new MapFunction<
                                        Triplet<Long, Point, Double>,
                                        Tuple3<Long, Long, Double>>() {

                                    @Override
                                    public Tuple3<Long, Long, Double> map(
                                            Triplet<Long, Point, Double> triplet) throws Exception {
                                        ...
                                        return new Tuple3<>(
                                                ...
                                                srcVertex
                                                        .getValue()
                                                        .euclideanDistance(trgVertex.getValue()));
                                    }
                                });

        Graph<Long, Point, Double> resultedGraph =
                graph.joinWithEdges(
                        edgesWithEuclideanWeight,
    ...
    private static boolean parseParameters(String[] args) {
        ...
        if (args.length > 0) {
            if (args.length == 3) {
                ...
                System.out.println(
                        "Executing Euclidean Graph Weighing example with default parameters and built-in default data.");
                System.out.println("Provide parameters to read input data from files.");
                System.out.println("See the documentation for the correct format of input files.");
                System.err.println(
                        "Usage: EuclideanGraphWeighing <input vertices path> <input edges path>"
                                + " <output path>");
                return false;
            }
        }
    ...
    private static DataSet<Vertex<Long, Point>> getVerticesDataSet(ExecutionEnvironment env) {
        if (fileOutput) {
            return env.readCsvFile(verticesInputPath)
                    ...
                    .map(
                            new MapFunction<Tuple3<Long, Double, Double>, Vertex<Long, Point>>() {

                                @Override
                                public Vertex<Long, Point> map(Tuple3<Long, Double, Double> value)
                                        throws Exception {
    ...
    private static DataSet<Edge<Long, Double>> getEdgesDataSet(ExecutionEnvironment env) {
        if (fileOutput) {
            return env.readCsvFile(edgesInputPath)
                    ...
                    .map(
                            new MapFunction<Tuple2<Long, Long>, Edge<Long, Double>>() {

                                @Override
                                public Edge<Long, Double> map(Tuple2<Long, Long> tuple2)
                                        throws Exception {
