...
class GraphOperationsITCase(mode: MultipleProgramsTestBase.TestExecutionMode)
  ...
  def testFilterOnVerticesSugar() {
    ...
    val graph: Graph[Long, Long, Long] = Graph.fromDataSet(
      ...
      TestGraphUtils.getLongLongEdgeData(env),
      env)
    val res = graph.filterOnVertices(vertex => vertex.getValue > 2).getEdges.collect().toList
  ...
  def testFilterOnEdgesSugar() {
    ...
    val graph: Graph[Long, Long, Long] = Graph.fromDataSet(
      ...
      TestGraphUtils.getLongLongEdgeData(env),
      env)
    val res = graph.filterOnEdges(edge => edge.getValue > 34).getEdges.collect().toList
