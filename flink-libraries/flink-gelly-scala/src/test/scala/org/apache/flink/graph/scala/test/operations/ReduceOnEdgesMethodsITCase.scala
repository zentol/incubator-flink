...
class ReduceOnEdgesMethodsITCase(mode: MultipleProgramsTestBase.TestExecutionMode)
  ...
  def testAllNeighborsWithValueGreaterThanFour {
    ...
    val graph: Graph[Long, Long, Long] = Graph.fromDataSet(
      ...
      env)
    val res = graph
      .groupReduceOnEdges(new SelectNeighborsValueGreaterThanFour, EdgeDirection.ALL)
      .collect()
      .toList
    expectedResult = "(5,1)\n" + "(5,3)\n" + "(5,4)"
  ...
  def testAllNeighbors {
    ...
    val graph: Graph[Long, Long, Long] = Graph.fromDataSet(
      ...
      env)
    val res = graph
      .groupReduceOnEdges(new SelectNeighbors, EdgeDirection.ALL)
      .collect()
      .toList
    expectedResult = "(1,2)\n" + "(1,3)\n" + "(1,5)\n" + "(2,1)\n" + "(2,3)\n" +
      "(3,1)\n" + "(3,2)\n" + "(3,4)\n" + "(3,5)\n" + "(4,3)\n" + "(4,5)\n" +
  ...
  final class SelectNeighborsValueGreaterThanFour
    extends EdgesFunctionWithVertexValue[Long, Long, Long, (Long, Long)] {
    @throws(classOf[Exception])
    override def iterateEdges(
        v: Vertex[Long, Long],
        edges: Iterable[Edge[Long, Long]],
  ...
  final class SelectNeighbors extends EdgesFunction[Long, Long, (Long, Long)] {
    @throws(classOf[Exception])
    override def iterateEdges(
        edges: Iterable[(Long, Edge[Long, Long])],
