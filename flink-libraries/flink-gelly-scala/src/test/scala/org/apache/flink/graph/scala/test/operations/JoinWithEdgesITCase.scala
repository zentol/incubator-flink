...
class JoinWithEdgesITCase(mode: MultipleProgramsTestBase.TestExecutionMode)
  ...
  def testWithEdgesInputDataset {
    ...
    val result: Graph[Long, Long, Long] =
      graph.joinWithEdges(graph.getEdges.map(new EdgeToTuple3Map[Long, Long]), new AddValuesMapper)
  ...
  def testWithEdgesInputDatasetSugar {
    ...
    val graph: Graph[Long, Long, Long] = Graph.fromDataSet(
      ...
      TestGraphUtils.getLongLongEdgeData(env),
      env)
    val result: Graph[Long, Long, Long] = graph.joinWithEdges(
      graph.getEdges.map(new EdgeToTuple3Map[Long, Long]),
  ...
  def testWithEdgesOnSource {
    ...
    val graph: Graph[Long, Long, Long] = Graph.fromDataSet(
      ...
      TestGraphUtils.getLongLongEdgeData(env),
      env)
    val result: Graph[Long, Long, Long] = graph.joinWithEdgesOnSource[Long](
      graph.getEdges
        .map(new ProjectSourceAndValueMapper),
      (originalValue: Long, tupleValue: Long) => originalValue + tupleValue)
    val res = result.getEdges.collect().toList
    expectedResult = "1,2,24\n" + "1,3,25\n" + "2,3,46\n" + "3,4,68\n" + "3,5,69\n" + "4,5," +
      "90\n" + "5,1,102\n"
  ...
  def testWithEdgesOnTarget {
    ...
    val graph: Graph[Long, Long, Long] = Graph.fromDataSet(
      ...
      TestGraphUtils.getLongLongEdgeData(env),
      env)
    val result: Graph[Long, Long, Long] = graph.joinWithEdgesOnTarget[Long](
      graph.getEdges
        .map(new ProjectTargetAndValueMapper),
      (originalValue: Long, tupleValue: Long) => originalValue + tupleValue)
    val res = result.getEdges.collect().toList
    expectedResult = "1,2,24\n" + "1,3,26\n" + "2,3,36\n" + "3,4,68\n" + "3,5,70\n" + "4,5," +
      "80\n" + "5,1,102\n"
