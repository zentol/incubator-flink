...
class GraphCreationWithCsvITCase(mode: MultipleProgramsTestBase.TestExecutionMode)
  ...
  def testCsvNoVertexValues() {
    ...
    val edgesSplit = createTempFile(edgesContent)
    val graph =
      Graph.fromCsvReader[Long, NullValue, Long](pathEdges = edgesSplit.getPath.toString, env = env)
  ...
  def createTempFile(content: String): FileInputSplit = {
    ...
    new FileInputSplit(0, new Path(tempFile.toURI.toString), 0, tempFile.length, Array("localhost"))
  }
