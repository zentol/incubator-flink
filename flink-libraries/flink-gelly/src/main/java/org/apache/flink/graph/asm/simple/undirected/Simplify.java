...
public class Simplify<K extends Comparable<K>, VV, EV>
    ...
    /**
     * Simplifies an undirected graph by adding reverse edges and removing self-loops and duplicate
     * edges.
     *
     * <p>When clip-and-flip is set, edges where source < target are removed before symmetrizing the
     * graph.
     *
    ...
    /**
     * Filter out edges where the source and target vertex IDs are equal and for each edge also emit
     * an edge with the vertex IDs flipped.
     *
     * @param <T> ID type
