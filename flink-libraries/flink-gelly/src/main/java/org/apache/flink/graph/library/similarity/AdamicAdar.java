 ...
 *
 * <p>Adamic-Adar measures the similarity between pairs of vertices as the sum of the inverse
 * logarithm of degree over shared neighbors. Scores are non-negative and unbounded. A vertex with
 * higher degree has greater overall influence but is less influential to each pair of neighbors.
 *
 * <p>This implementation produces similarity scores for each pair of vertices in the graph with at
 * least one shared neighbor; equivalently, this is the set of all non-zero Adamic-Adar
 * coefficients.
 *
...
public class AdamicAdar<K extends CopyableValue<K>, VV, EV>
    ...
    private static final String SUM_OF_SCORES_AND_NUMBER_OF_NEIGHBOR_PAIRS =
    ...
    public DataSet<Result<K>> runInternal(Graph<K, VV, EV> input) throws Exception {
        ...
        DataSet<Tuple3<K, LongValue, FloatValue>> inverseLogDegree =
                input.run(new VertexDegree<K, VV, EV>().setParallelism(parallelism))
                        .map(new VertexInverseLogDegree<>())
                        .setParallelism(parallelism)
                        .name("Vertex score");

        // s, t, 1/log(d(s))
        DataSet<Tuple3<K, K, FloatValue>> sourceInverseLogDegree =
                input.getEdges()
        ...
        // group, s, t, 1/log(d(s))
        DataSet<Tuple4<IntValue, K, K, FloatValue>> groups =
                groupSpans
                        .rebalance()
                        .setParallelism(parallelism)
                        .name("Rebalance")
        ...
        DataSet<Tuple3<K, K, FloatValue>> twoPaths =
                groups.groupBy(0, 1)
        ...
        if (minimumRatio > 0.0f) {
            ...
            DataSet<Tuple2<FloatValue, LongValue>> sumOfScoresAndNumberOfNeighborPairs =
                    inverseLogDegree
            ...
            scores.withBroadcastSet(
                    sumOfScoresAndNumberOfNeighborPairs,
                    SUM_OF_SCORES_AND_NUMBER_OF_NEIGHBOR_PAIRS);
        }

        if (mirrorResults) {
            return scores.flatMap(new MirrorResult<>()).name("Mirror results");
        } else {
            return scores;
        }
    }
    ...
    /**
     * Compute the inverse logarithm of the vertex degree. This is computed before enumerating
     * neighbor pairs since logarithm and division are quite computationally intensive.
     *
     * @param <T> ID type
     */
    @ForwardedFields("0; 1")
    private static class VertexInverseLogDegree<T>
        ...
        private Tuple3<T, LongValue, FloatValue> output =
                new Tuple3<>(null, null, new FloatValue());
        ...
        public Tuple3<T, LongValue, FloatValue> map(Vertex<T, LongValue> value) throws Exception {
            ...
            float inverseLogDegree =
                    (degree == 1) ? 0.0f : 1.0f / (float) Math.log(value.f1.getValue());
    ...
    private static class GenerateGroupSpans<T>
            implements GroupReduceFunction<
                    Tuple3<T, T, FloatValue>, Tuple4<IntValue, T, T, FloatValue>> {
        private IntValue groupSpansValue = new IntValue();

        private Tuple4<IntValue, T, T, FloatValue> output =
                new Tuple4<>(groupSpansValue, null, null, null);

        @Override
        public void reduce(
                Iterable<Tuple3<T, T, FloatValue>> values,
                Collector<Tuple4<IntValue, T, T, FloatValue>> out)
                throws Exception {
            int groupCount = 0;
            int groupSpans = 1;
    ...
    private static class GenerateGroups<T>
            implements FlatMapFunction<
                    Tuple4<IntValue, T, T, FloatValue>, Tuple4<IntValue, T, T, FloatValue>> {
        @Override
        public void flatMap(
                Tuple4<IntValue, T, T, FloatValue> value,
                Collector<Tuple4<IntValue, T, T, FloatValue>> out)
                throws Exception {
    ...
    private static class GenerateGroupPairs<T extends CopyableValue<T>>
            implements GroupReduceFunction<
                    Tuple4<IntValue, T, T, FloatValue>, Tuple3<T, T, FloatValue>> {
        ...
        @Override
        public void reduce(
                Iterable<Tuple4<IntValue, T, T, FloatValue>> values,
                Collector<Tuple3<T, T, FloatValue>> out)
                throws Exception {
            int visitedCount = 0;
    ...
    private static class ComputeScoreFromVertex<T>
            implements MapFunction<
                    Tuple3<T, LongValue, FloatValue>, Tuple2<FloatValue, LongValue>> {
        ...
        private Tuple2<FloatValue, LongValue> output =
                new Tuple2<>(sumOfScores, numberOfNeighborPairs);

        @Override
        public Tuple2<FloatValue, LongValue> map(Tuple3<T, LongValue, FloatValue> value)
                throws Exception {
            long degree = value.f1.getValue();
    ...
    /**
     * Compute the Adamic-Adar similarity as the sum over common neighbors of the inverse logarithm
     * of degree.
     *
     * @param <T> ID type
     */
    ...
    private static class ComputeScores<T>
        ...
        public void open(Configuration parameters) throws Exception {
            ...
            if (minimumRatio > 0.0f) {
                ...
                var =
                        getRuntimeContext()
                                .getBroadcastVariable(SUM_OF_SCORES_AND_NUMBER_OF_NEIGHBOR_PAIRS);
    ...
    public static class Result<T> extends BinaryResultBase<T>
        ...
        /**
         * Get the Adamic-Adar score, equal to the sum over common neighbors of the inverse
         * logarithm of degree.
         *
        ...
        /**
         * Set the Adamic-Adar score, equal to the sum over common neighbors of the inverse
         * logarithm of degree.
         *
        ...
        public String toString() {
            return "(" + getVertexId0() + "," + getVertexId1() + "," + adamicAdarScore + ")";
        }
