...
/**
 * The base class for the message-passing functions between vertices as a part of a {@link
...
public abstract class ComputeFunction<K, VV, EV, Message> implements Serializable {
    ...
    /**
     * This method is invoked once per superstep, for each active vertex. A vertex is active during
     * a superstep, if at least one message was produced for it, in the previous superstep. During
     * the first superstep, all vertices are active.
     *
     * <p>This method can iterate over all received messages, set the new vertex value, and send
     * messages to other vertices (which will be delivered in the next superstep).
     *
    ...
    /**
     * Gets an {@link java.lang.Iterable} with all out-going edges. This method is mutually
     * exclusive with {@link #sendMessageToAllNeighbors(Object)} and may be called only once.
     *
    ...
    /**
     * Sends the given message to all vertices that adjacent to the changed vertex. This method is
     * mutually exclusive to the method {@link #getEdges()} and may be called only once.
     *
    ...
    /**
     * Sends the given message to the vertex identified by the given key. If the target vertex does
     * not exist, the next superstep will cause an exception due to a non-deliverable message.
     *
     ...
     * @return The aggregator registered under this name, or {@code null}, if no aggregator was
     *     registered.
     */
     ...
     * org.apache.flink.graph.pregel.VertexCentricConfiguration#addBroadcastSet(String, DataSet)}.
     *
