...
public class GatherSumApplyIteration<K, VV, EV, M>
    ...
    private GatherSumApplyIteration(
    ...
    public DataSet<Vertex<K, VV>> createResult() {
        ...
        TypeInformation<M> messageType =
                TypeExtractor.createTypeInfo(gather, GatherFunction.class, gather.getClass(), 2);
            ...
            case OUT:
                neighbors =
                        iteration
                                ...
                                .where(0)
                                .equalTo(0)
                                .with(new ProjectKeyWithNeighborOUT<>());
                break;
            case IN:
                neighbors =
                        iteration
                                ...
                                .where(0)
                                .equalTo(1)
                                .with(new ProjectKeyWithNeighborIN<>());
                break;
            case ALL:
                neighbors =
                        iteration
        ...
        MapOperator<Tuple2<K, Neighbor<VV, EV>>, Tuple2<K, M>> gatherMapOperator =
                neighbors.map(gatherUdf);
        ...
        JoinOperator<?, ?, Vertex<K, VV>> appliedSet =
                summedSet.join(iteration.getSolutionSet()).where(0).equalTo(0).with(applyUdf);
    ...
    private static final class GatherUdf<K, VV, EV, M>
        ...
        private GatherUdf(
                ...
                TypeInformation<Tuple2<K, M>> resultType) {
    ...
    private static final class SumUdf<K, VV, EV, M> extends RichReduceFunction<Tuple2<K, M>>
        ...
        private SumUdf(
                SumFunction<VV, EV, M> sumFunction, TypeInformation<Tuple2<K, M>> resultType) {
    ...
    private static final class ApplyUdf<K, VV, EV, M>
        ...
        private ApplyUdf(
                ApplyFunction<K, VV, M> applyFunction, TypeInformation<Vertex<K, VV>> resultType) {
        ...
        public void join(
                Tuple2<K, M> newValue,
                final Vertex<K, VV> currentValue,
                final Collector<Vertex<K, VV>> out)
                throws Exception {
        ...
        public void open(Configuration parameters) throws Exception {
            if (getRuntimeContext().hasBroadcastVariable("number of vertices")) {
                Collection<LongValue> numberOfVertices =
                        getRuntimeContext().getBroadcastVariable("number of vertices");
                this.applyFunction.setNumberOfVertices(
                        numberOfVertices.iterator().next().getValue());
            }
            if (getIterationRuntimeContext().getSuperstepNumber() == 1) {
