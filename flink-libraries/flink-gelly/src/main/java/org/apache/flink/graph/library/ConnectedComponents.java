 ...
 *
 * <p>This implementation uses a comparable vertex value as initial component identifier (ID).
 * Vertices propagate their current value in each iteration. Upon receiving component IDs from its
 * neighbors, a vertex adopts a new component ID if its value is lower than its current component
 * ID.
 *
...
public class ConnectedComponents<K, VV extends Comparable<VV>, EV>
    ...
    /**
     * Creates an instance of the Connected Components algorithm. The algorithm computes weakly
    ...
    public DataSet<Vertex<K, VV>> run(Graph<K, VV, EV> graph) throws Exception {
        ...
        return undirectedGraph
                .runScatterGatherIteration(
                        new CCMessenger<>(valueTypeInfo), new CCUpdater<>(), maxIterations)
                .getVertices();
    }
    ...
    /**
     * Updates the value of a vertex by picking the minimum neighbor value out of all the incoming
     * messages.
     */
