...
/**
 * A class to build a Graph using path(s) provided to CSV file(s) with optional vertex and edge
 * data. The class also configures the CSV readers used to read edge and vertex data such as the
 * field types, the delimiters (row and field), the fields that should be included or skipped, and
 * other flags, such as whether to skip the initial line as the header. The configuration is done
 * using the functions provided in the {@link org.apache.flink.api.java.io.CsvReader} class.
 */
public class GraphCsvReader {
    ...
    public <K, VV> GraphCsvReader(
            Path edgePath, final MapFunction<K, VV> mapper, ExecutionEnvironment context) {
        this.vertexPath = null;
        this.edgePath = edgePath;
        this.edgeReader = new CsvReader(edgePath, context);
        this.vertexReader = null;
    ...
    public GraphCsvReader(String edgePath, ExecutionEnvironment context) {
        this(
                new Path(Preconditions.checkNotNull(edgePath, "The file path may not be null.")),
                context);
    }

    public GraphCsvReader(String vertexPath, String edgePath, ExecutionEnvironment context) {
        this(
                new Path(Preconditions.checkNotNull(vertexPath, "The file path may not be null.")),
                new Path(Preconditions.checkNotNull(edgePath, "The file path may not be null.")),
                context);
    }

    public <K, VV> GraphCsvReader(
            String edgePath, final MapFunction<K, VV> mapper, ExecutionEnvironment context) {
        this(
                new Path(Preconditions.checkNotNull(edgePath, "The file path may not be null.")),
                mapper,
                context);
    ...
    /**
     * Creates a Graph from CSV input with vertex values and edge values. The vertex values are
     * specified through a vertices input file or a user-defined map function.
     *
    ...
    public <K, VV, EV> Graph<K, VV, EV> types(
    ...
    public <K> Graph<K, NullValue, NullValue> keyType(Class<K> vertexKey) {
        ...
        DataSet<Edge<K, NullValue>> edges =
                edgeReader
    ...
    /**
     * Creates a Graph from CSV input without edge values. The vertex values are specified through a
     * vertices input file or a user-defined map function. If no vertices input file is provided,
     * the vertex IDs are automatically created from the edges input file.
     *
     * @param vertexKey the type of the vertex IDs
     * @param vertexValue the type of the vertex values
     ...
     * vertices ({@code ','}) is used by default.
     *
    ...
    /**
     * Enables quoted String parsing for Edge Csv Reader. Field delimiters in quoted Strings are
     * ignored. A String is parsed as quoted if it starts and ends with a quoting character and as
     * unquoted otherwise. Leading or tailing whitespaces are not allowed.
     *
    ...
    /**
     * Enables quoted String parsing for Vertex Csv Reader. Field delimiters in quoted Strings are
     * ignored. A String is parsed as quoted if it starts and ends with a quoting character and as
     * unquoted otherwise. Leading or tailing whitespaces are not allowed.
     *
     * @param quoteCharacter The character which is used as quoting character.
    ...
    /**
     * Configures the string that starts comments for the Vertex Csv Reader. By default comments
     * will be treated as invalid lines. This function only recognizes comments which start at the
    ...
    /**
     * Configures the string that starts comments for the Edge Csv Reader. By default comments will
     * be treated as invalid lines. This function only recognizes comments which start at the
     * beginning of the line!
     *
     * @param commentPrefix The string that starts the comments.
     * @return The GraphCSVReader instance itself, to allow for fluent function chaining.
     ...
     * should be skipped. The parser will look at the first {@code n} fields, where {@code n} is the
     * length of the boolean array. The parser will skip over all fields where the boolean value at
     * the corresponding position in the array is {@code false}. The result contains the fields
     * where the corresponding position in the boolean array is {@code true}. The number of fields
     * in the result is consequently equal to the number of times that {@code true} occurs in the
     * fields array.
     *
     * @param vertexFields The array of flags that describes which fields are to be included from
     *     the CSV file for vertices.
     * @return The GraphCSVReader instance itself, to allow for fluent function chaining.
     */
     ...
     * fields array.
     *
     * @param edgeFields The array of flags that describes which fields are to be included from the
     *     CSV file for edges.
     * @return The GraphCSVReader instance itself, to allow for fluent function chaining.
     */
    ...
    /**
     * Configures which fields of the CSV file containing vertices data should be included and which
     * should be skipped. The positions in the string (read from position 0 to its length) define
     * whether the field at the corresponding position in the CSV schema should be included. parser
     * will look at the first {@code n} fields, where {@code n} is the length of the mask string The
     * parser will skip over all fields where the character at the corresponding position in the
     * string is {@code '0'}, {@code 'F'}, or {@code 'f'} (representing the value {@code false}).
     * The result contains the fields where the corresponding position in the boolean array is
     * {@code '1'}, {@code 'T'}, or {@code 't'} (representing the value {@code true}).
     *
    ...
    /**
     * Configures which fields of the CSV file containing vertices data should be included and which
     * should be skipped. The bits in the value (read from least significant to most significant)
     * define whether the field at the corresponding position in the CSV schema should be included.
     * parser will look at the first {@code n} fields, where {@code n} is the position of the most
    ...
    /**
     * Sets the CSV reader for the Edges file to ignore the first line. This is useful for files
     * that contain a header line.
     *
     * @return The GraphCSVReader instance itself, to allow for fluent function chaining.
     */
    ...
    /**
     * Sets the CSV reader for the Vertices file to ignore the first line. This is useful for files
     * that contain a header line.
     *
     * @return The GraphCSVReader instance itself, to allow for fluent function chaining.
     */
    ...
    /**
     * Sets the CSV reader for the Edges file to ignore any invalid lines. This is useful for files
     * that contain an empty line at the end, multiple header lines or comments. This would throw an
     * exception otherwise.
     *
     * @return The GraphCSVReader instance itself, to allow for fluent function chaining.
     */
    ...
    /**
     * Sets the CSV reader Vertices file to ignore any invalid lines. This is useful for files that
     * contain an empty line at the end, multiple header lines or comments. This would throw an
     * exception otherwise.
     *
     * @return The GraphCSVReader instance itself, to allow for fluent function chaining.
     */
