 ...
 *
 * <p>Strings are serialized to a byte array. Concatenating arrays is as simple and fast as
 * extending and copying byte arrays. Strings are serialized when individually added to {@code
 * StringValueArray}.
 *
 * <p>For each string added to the array the length is first serialized using a variable length
 * integer. Then the string characters are serialized using a variable length encoding where the
 * lower 128 ASCII/UFT-8 characters are encoded in a single byte. This ensures that common
 * characters are serialized in only two bytes.
 */
public class StringValueArray implements ValueArray<StringValue> {
    ...
    private void initialize(int bytes) {
        ...
        Preconditions.checkArgument(
                bytes <= MAX_ARRAY_SIZE, "Requested capacity exceeds limit of " + MAX_ARRAY_SIZE);
    ...
    private class ReadIterator implements Iterator<StringValue> {
