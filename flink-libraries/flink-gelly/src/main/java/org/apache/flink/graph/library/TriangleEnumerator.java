...
/**
 * This library method enumerates unique triangles present in the input graph. A triangle consists
 * of three edges that connect three vertices with each other. Edge directions are ignored here. The
 * method returns a DataSet of Tuple3, where the fields of each Tuple3 contain the Vertex IDs of a
 * triangle.
 *
 * <p>The basic algorithm groups all edges that share a common vertex and builds triads, i.e.,
 * triples of vertices that are connected by two edges. Then all triads are filtered for which no
 * third edge exists that closes the triangle.
 *
 * <p>For a group of <i>n</i> edges that share a common vertex, the number of built triads is
 * quadratic <i>((n*(n-1))/2)</i>. Therefore, an optimization of the algorithm is to group edges on
 * the vertex with the smaller output degree to reduce the number of triads.
 *
 * <p>This implementation extends the basic algorithm by computing output degrees of edge vertices
 * and grouping on edges on the vertex with the smaller degree.
 */
public class TriangleEnumerator<K extends Comparable<K>, VV, EV>
    ...
    public DataSet<Tuple3<K, K, K>> run(Graph<K, VV, EV> input) throws Exception {
        ...
        DataSet<EdgeWithDegrees<K>> edgesWithDegrees =
                edges.flatMap(new EdgeDuplicator<>())
                        ...
                        .reduceGroup(new DegreeCounter<>())
                        .groupBy(EdgeWithDegrees.V1, EdgeWithDegrees.V2)
                        .reduce(new DegreeJoiner<>());
        ...
        DataSet<Tuple3<K, K, K>> triangles =
                edgesByDegree
                        // build triads
                        .groupBy(EdgeWithDegrees.V1)
                        .sortGroup(EdgeWithDegrees.V2, Order.ASCENDING)
                        .reduceGroup(new TriadBuilder<>())
                        // filter triads
                        .join(edgesById, JoinHint.REPARTITION_HASH_SECOND)
    ...
    /**
     * Counts the number of edges that share a common vertex. Emits one edge for each input edge
     * with a degree annotation for the shared vertex. For each emitted edge, the first vertex is
     * the vertex with the smaller id.
     */
    @SuppressWarnings("serial")
    ...
    private static final class DegreeJoiner<K> implements ReduceFunction<EdgeWithDegrees<K>> {
        ...
        @Override
        public EdgeWithDegrees<K> reduce(EdgeWithDegrees<K> edge1, EdgeWithDegrees<K> edge2)
                throws Exception {
    ...
    /**
     * Projects an edge (pair of vertices) such that the first vertex is the vertex with the smaller
     * degree.
     */
    @SuppressWarnings("serial")
    ...
    private static final class TriadBuilder<K extends Comparable<K>>
        ...
        @Override
        public void reduce(Iterable<Edge<K, NullValue>> edgesIter, Collector<Triad<K>> out)
                throws Exception {
    ...
    private static final class TriadFilter<K>
        ...
        public Tuple3<K, K, K> join(Triad<K> triad, Edge<K, NullValue> edge) throws Exception {
            return new Tuple3<>(
                    triad.getFirstVertex(), triad.getSecondVertex(), triad.getThirdVertex());
        }
    }
