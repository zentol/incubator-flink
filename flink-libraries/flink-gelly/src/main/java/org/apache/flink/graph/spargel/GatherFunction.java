...
/**
 * This class must be extended by functions that compute the state of the vertex depending on the
 * old state and the incoming messages. The central method is {@link #updateVertex(Vertex,
 * MessageIterator)}, which is invoked once per vertex per superstep.
 *
 * <p>{@code <K>} The vertex key type. {@code <VV>} The vertex value type. {@code <Message>} The
 * message type.
 */
public abstract class GatherFunction<K, VV, Message> implements Serializable {
    ...
    // --------------------------------------------------------------------------------------------
    //  Attributes that allow vertices to access their in/out degrees and the total number of
    // vertices
    //  inside an iteration.
    // --------------------------------------------------------------------------------------------
     ...
     *     org.apache.flink.graph.IterationConfiguration#setOptNumVertices(boolean)} option has been
     *     set; -1 otherwise.
     */
    ...
    /**
     * This method is invoked once per vertex per superstep. It receives the current state of the
     * vertex, as well as the incoming messages. It may set a new vertex state via {@link
     * #setNewVertexValue(Object)}. If the vertex state is changed, it will trigger the sending of
     * messages via the {@link ScatterFunction}.
     *
    ...
    /**
     * This method is executed once per superstep after the gather function has been invoked for
     * each vertex.
     *
    ...
    /**
     * Sets the new value of this vertex. Setting a new value triggers the sending of outgoing
     * messages from this vertex.
     *
     ...
     * combines all aggregates globally once per superstep and makes them available in the next
     * superstep.
     *
    ...
    /**
     * Gets the broadcast data set registered under the given name. Broadcast data sets are
     * available on all parallel instances of a function. They can be registered via {@link
     ...
     *
     * <p>This function will retrieve the vertex from the vertexState and will set its degrees,
     * afterwards calling the regular updateVertex function.
     *
     * @param vertexState
     * @param inMessages
     * @throws Exception
    ...
    <VertexWithDegree> void updateVertexFromScatterGatherIteration(
