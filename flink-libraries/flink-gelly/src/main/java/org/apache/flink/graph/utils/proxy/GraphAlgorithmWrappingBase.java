...
/**
 * A {@link GraphAlgorithm} transforms an input {@link Graph} into an output of type {@code T}.
 * Subclasses of {@link GraphAlgorithmWrappingBase} wrap the output with a {@code NoOpOperator}. The
 * input to the wrapped operators can be replaced when the same algorithm is run on the same input
 * with a mergeable configuration. This allows algorithms to be composed of implicitly reusable
...
public abstract class GraphAlgorithmWrappingBase<K, VV, EV, R>
    ...
    /**
     * Set the parallelism for this algorithm's operators. This parameter is necessary because
     ...
     *
     * @return name of the algorithm, which may be shared by multiple classes implementing the same
     *     algorithm and generating the same output
     */
    ...
    /**
     * First test whether the algorithm configurations can be merged before the call to {@link
     * #mergeConfiguration}.
     *
     * @param other the algorithm with which to compare configuration
     * @return true if and only if configuration can be merged and the algorithm's output can be
     *     reused
    ...
    /**
     * Merge the other configuration into this algorithm's after the call to {@link
     * #canMergeConfigurationWith} has checked that the configurations can be merged.
     *
    ...
    protected void mergeConfiguration(GraphAlgorithmWrappingBase other) {
        ...
        parallelism =
                (parallelism == PARALLELISM_DEFAULT)
                        ? other.parallelism
                        : ((other.parallelism == PARALLELISM_DEFAULT)
                                ? parallelism
                                : Math.min(parallelism, other.parallelism));
    }
}
