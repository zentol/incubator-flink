...
/**
 * PageRank computes a per-vertex score which is the sum of PageRank scores transmitted over
 * in-edges. Each vertex's score is divided evenly among out-edges. High-scoring vertices are linked
 * to by other high-scoring vertices; this is similar to the 'authority' score in {@link HITS}.
 *
...
public class PageRank<K, VV, EV> extends GraphAlgorithmWrappingDataSet<K, VV, EV, Result<K>> {
    ...
    /**
     * PageRank with a convergence threshold. The algorithm terminates when the change in score over
     * all vertices falls to or below the given threshold value.
     *
     * @param dampingFactor probability of following an out-link, otherwise jump to a random vertex
    ...
    /**
     * PageRank with a convergence threshold and a maximum iteration count. The algorithm terminates
     * after either the given number of iterations or when the change in score over all vertices
     * falls to or below the given threshold value.
     *
     * @param dampingFactor probability of following an out-link, otherwise jump to a random vertex
    ...
    public PageRank(double dampingFactor, int maxIterations, double convergenceThreshold) {
        Preconditions.checkArgument(
                0 < dampingFactor && dampingFactor < 1,
     ...
     *
     * <p>Setting this flag includes "zero-degree" vertices in the PageRank computation and result.
     * These vertices are handled the same as other "source" vertices (with a consistent score of
     * <code>(1 - damping factor) / number of vertices</code>) but only affect the scores of other
     * vertices indirectly through the taking of this proportional portion of the "random jump"
     * score.
     *
     * <p>The cost to include zero-degree vertices is a reduce for uniqueness on the vertex set
     * followed by an outer join on the vertex degree DataSet.
     *
     * @param includeZeroDegreeVertices whether to include zero-degree vertices in the iterative
     *     computation
     * @return this
     */
    ...
    public DataSet<Result<K>> runInternal(Graph<K, VV, EV> input) throws Exception {
        ...
        // s, t, d(s)
        DataSet<Edge<K, LongValue>> edgeSourceDegree =
        ...
        DataSet<Tuple2<K, DoubleValue>> sourceVertices =
                vertexDegree
        ...
        DataSet<Tuple2<K, DoubleValue>> initialScores =
                vertexDegree
        ...
        IterativeDataSet<Tuple2<K, DoubleValue>> iterative =
                initialScores.iterate(maxIterations).setParallelism(parallelism);

        // s, projected pagerank(s)
        DataSet<Tuple2<K, DoubleValue>> vertexScores =
                iterative
        ...
        DataSet<Tuple2<K, DoubleValue>> sumOfScores =
                vertexScores
        ...
        DataSet<Tuple2<K, DoubleValue>> adjustedScores =
                vertexScores
    ...
    private static class ExtractSourceDegree<T, ET>
        ...
        @Override
        public Edge<T, LongValue> map(Edge<T, Tuple2<ET, Degrees>> edge) throws Exception {
            output.f0 = edge.f0;
            output.f1 = edge.f1;
    ...
    /**
     * PageRank scores sum to 1.0 so initialize each vertex with the inverse of the number of
     * vertices.
     *
     * @param <T> ID type
     */
    ...
    private static class InitializeVertexScores<T>
        ...
        public void open(Configuration parameters) throws Exception {
            ...
            output.f1 =
                    new DoubleValue(
                            vertexCountIterator.hasNext()
                                    ? 1.0 / vertexCountIterator.next().getValue()
                                    : Double.NaN);
        }

        @Override
        public Tuple2<T, DoubleValue> map(Vertex<T, Degrees> vertex) throws Exception {
            output.f0 = vertex.f0;
            return output;
        }
    }
    ...
    /**
     * The PageRank score for each vertex is divided evenly and projected to neighbors on out-edges.
     *
     * @param <T> ID type
     */
    ...
    /**
     * Sum the PageRank score over all vertices. The vertex ID must be ignored but is retained
     * rather than adding another operator.
     *
     * @param <T> ID type
     */
    @ForwardedFields("0")
    private static class SumVertexScores<T> implements ReduceFunction<Tuple2<T, DoubleValue>> {
        ...
        public Tuple2<T, DoubleValue> reduce(
                Tuple2<T, DoubleValue> first, Tuple2<T, DoubleValue> second) throws Exception {
    ...
    /**
     * Each iteration the per-vertex scores are adjusted with the damping factor. Each score is
     * multiplied by the damping factor then added to the probability of a "random hop", which is
     * one minus the damping factor.
     *
     * <p>This operation also accounts for 'sink' vertices, which have no out-edges to project score
     * to. The sink scores are computed by taking one minus the sum of vertex scores, which also
     * includes precision error. This 'missing' score is evenly distributed across vertices as with
     * the random hop.
     *
     * @param <T> ID type
     */
    ...
    private static class AdjustScores<T>
        ...
        public void open(Configuration parameters) throws Exception {
            ...
            double sumOfSinks =
                    1
                            - (sumOfScoresIterator.hasNext()
                                    ? sumOfScoresIterator.next().f1.getValue()
                                    : 0);
            ...
            Iterator<LongValue> vertexCountIterator = vertexCount.iterator();
            this.vertexCount =
                    vertexCountIterator.hasNext() ? vertexCountIterator.next().getValue() : 0;

            this.uniformlyDistributedScore =
                    ((1 - dampingFactor) + dampingFactor * sumOfSinks) / this.vertexCount;
        }
    ...
    /**
     * Computes the sum of the absolute change in vertex PageRank scores between iterations.
     *
     * @param <T> ID type
     */
    ...
    private static class ChangeInScores<T>
            extends RichJoinFunction<
                    Tuple2<T, DoubleValue>, Tuple2<T, DoubleValue>, Tuple2<T, DoubleValue>> {
        ...
        @Override
        public Tuple2<T, DoubleValue> join(
                Tuple2<T, DoubleValue> first, Tuple2<T, DoubleValue> second) throws Exception {
    ...
    /**
     * Monitors the sum of the absolute change in vertex scores. The algorithm terminates when the
     * change in scores compared against the prior iteration falls to or below the given convergence
     * threshold.
     */
    ...
    public static class Result<T> extends UnaryResultBase<T> implements PrintableResult {
        ...
        public String toString() {
            return "(" + getVertexId0() + "," + pageRankScore + ")";
        }

        @Override
        public String toPrintableString() {
            return "Vertex ID: " + getVertexId0() + ", PageRank score: " + pageRankScore;
        }
