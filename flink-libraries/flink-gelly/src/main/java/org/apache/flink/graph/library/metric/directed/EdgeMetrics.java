...
/**
 * Compute the following edge metrics in a directed graph. - number of triangle triplets - number of
 * rectangle triplets - maximum number of triangle triplets - maximum number of rectangle triplets
 *
...
public class EdgeMetrics<K extends Comparable<K>, VV, EV>
    ...
    /**
     * Produces a pair of tuples. The first tuple contains the source vertex ID, the target vertex
     * ID, the source degrees, and the low-order count. The second tuple is the same with the source
     * and target roles reversed.
     *
     * <p>The low-order count is one if the source vertex degree is less than the target vertex
     * degree or if the degrees are equal and the source vertex ID compares lower than the target
     * vertex ID; otherwise the low-order count is zero.
     *
     * @param <T> ID type
    ...
    private static final class EdgeStats<T extends Comparable<T>, ET>
            implements FlatMapFunction<
                    Edge<T, Tuple3<ET, Degrees, Degrees>>, Tuple4<T, T, Degrees, LongValue>> {
        ...
        @Override
        public void flatMap(
                Edge<T, Tuple3<ET, Degrees, Degrees>> edge,
    ...
    private static final class ReduceEdgeStats<T>
            implements GroupReduceFunction<
                    Tuple4<T, T, Degrees, LongValue>, Tuple3<T, Degrees, LongValue>> {
        Tuple3<T, Degrees, LongValue> output = new Tuple3<>();

        @Override
        public void reduce(
                Iterable<Tuple4<T, T, Degrees, LongValue>> values,
                Collector<Tuple3<T, Degrees, LongValue>> out)
                throws Exception {
     ...
     * @param <T> ID type
     */
    private static class SumEdgeStats<T> implements ReduceFunction<Tuple3<T, Degrees, LongValue>> {
        @Override
        public Tuple3<T, Degrees, LongValue> reduce(
                Tuple3<T, Degrees, LongValue> value1, Tuple3<T, Degrees, LongValue> value2)
                throws Exception {
