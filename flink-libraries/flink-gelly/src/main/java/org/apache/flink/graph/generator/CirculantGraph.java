...
/**
 * @see <a href="http://mathworld.wolfram.com/CirculantGraph.html">Circulant Graph at Wolfram
 *     MathWorld</a>
 */
public class CirculantGraph extends GraphGeneratorBase<LongValue, NullValue, NullValue> {
    ...
    /**
     * An oriented {@link Graph} with {@code n} vertices where each vertex v<sub>i</sub> is
     * connected to vertex v<sub>(i+j)%n</sub> for each configured offset {@code j}.
     *
    ...
    public CirculantGraph addRange(long offset, long length) {
        Preconditions.checkArgument(
                offset >= MINIMUM_OFFSET, "Range offset must be at least " + MINIMUM_OFFSET);
        Preconditions.checkArgument(
        ...
        while (iter.hasNext()) {
            ...
            if (lastRange.overlaps(nextRange)) {
                throw new IllegalArgumentException(
                        "Overlapping ranges " + lastRange + " and " + nextRange);
            }
