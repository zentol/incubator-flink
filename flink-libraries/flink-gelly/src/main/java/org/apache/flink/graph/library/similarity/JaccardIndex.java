 ...
 * number of shared neighbors divided by the number of distinct neighbors. Scores range from 0.0 (no
 * shared neighbors) to 1.0 (all neighbors are shared).
 *
 * <p>This implementation produces similarity scores for each pair of vertices in the graph with at
 * least one shared neighbor; equivalently, this is the set of all non-zero Jaccard Similarity
 * coefficients.
 *
...
public class JaccardIndex<K extends CopyableValue<K>, VV, EV>
    ...
    /**
     * Override the default group size for the quadratic expansion of neighbor pairs. Small groups
     * generate more data whereas large groups distribute computation less evenly among tasks.
     *
    ...
    public JaccardIndex<K, VV, EV> setMaximumScore(int numerator, int denominator) {
        ...
        Preconditions.checkArgument(
                denominator > 0, "Maximum score denominator must be greater than zero");
        Preconditions.checkArgument(
                numerator <= denominator,
                "Maximum score fraction must be less than or equal to one");
    ...
    /**
     * By default only one result is output for each pair of vertices. When mirroring a second
     * result with the vertex order flipped is output for each pair of vertices.
     *
    ...
    public DataSet<Result<K>> runInternal(Graph<K, VV, EV> input) throws Exception {
        // s, t, d(t)
        DataSet<Edge<K, Tuple2<EV, LongValue>>> neighborDegree =
                input.run(new EdgeTargetDegree<K, VV, EV>().setParallelism(parallelism));
        ...
        // group, s, t, d(t)
        DataSet<Tuple4<IntValue, K, K, IntValue>> groups =
                groupSpans
                        .rebalance()
                        .setParallelism(parallelism)
                        .name("Rebalance")
        ...
        // t, u, d(t)+d(u)
        DataSet<Tuple3<K, K, IntValue>> twoPaths =
                groups.groupBy(0, 1)
        ...
        if (mirrorResults) {
            scores = scores.flatMap(new MirrorResult<>()).name("Mirror results");
        }
    ...
    /**
     * This is the first of three operations implementing a self-join to generate the full neighbor
     * pairing for each vertex. The number of neighbor pairs is (n choose 2) which is quadratic in
     * the vertex degree.
     *
     * <p>The third operation, {@link GenerateGroupPairs}, processes groups of size {@link
     * #groupSize} and emits {@code O(groupSize * deg(vertex))} pairs.
     *
     * <p>This input to the third operation is still quadratic in the vertex degree. Two prior
     * operations, {@link GenerateGroupSpans} and {@link GenerateGroups}, each emit datasets linear
     * in the vertex degree, with a forced rebalance in between. {@link GenerateGroupSpans} first
     * annotates each edge with the number of groups and {@link GenerateGroups} emits each edge into
     * each group.
     *
     * @param <T> ID type
    ...
    private static class GenerateGroupSpans<T, ET>
            implements GroupReduceFunction<
                    Edge<T, Tuple2<ET, LongValue>>, Tuple4<IntValue, T, T, IntValue>> {
        private final int groupSize;

        private IntValue groupSpansValue = new IntValue();

        private Tuple4<IntValue, T, T, IntValue> output =
        ...
        @Override
        public void reduce(
                Iterable<Edge<T, Tuple2<ET, LongValue>>> values,
                Collector<Tuple4<IntValue, T, T, IntValue>> out)
                throws Exception {
            int groupCount = 0;
            int groupSpans = 1;
    ...
    private static class GenerateGroups<T>
            implements FlatMapFunction<
                    Tuple4<IntValue, T, T, IntValue>, Tuple4<IntValue, T, T, IntValue>> {
        @Override
        public void flatMap(
                Tuple4<IntValue, T, T, IntValue> value,
                Collector<Tuple4<IntValue, T, T, IntValue>> out)
                throws Exception {
     ...
     *
     * <p>The first {@link #groupSize} vertices are emitted pairwise. Following vertices are only
     * paired with vertices from this initial group.
     *
     * @param <T> ID type
     * @see GenerateGroupSpans
     */
    private static class GenerateGroupPairs<T extends CopyableValue<T>>
            implements GroupReduceFunction<
                    Tuple4<IntValue, T, T, IntValue>, Tuple3<T, T, IntValue>> {
        private final int groupSize;
        ...
        @Override
        public void reduce(
                Iterable<Tuple4<IntValue, T, T, IntValue>> values,
                Collector<Tuple3<T, T, IntValue>> out)
                throws Exception {
            int visitedCount = 0;
    ...
    /**
     * Compute the counts of shared and distinct neighbors. A two-path connecting the vertices is
     * emitted for each shared neighbor. The number of distinct neighbors is equal to the sum of
     * degrees of the vertices minus the count of shared numbers, which are double-counted in the
     * degree sum.
     *
     * @param <T> ID type
     */
    ...
    private static class ComputeScores<T>
        ...
        public ComputeScores(
                boolean unboundedScores,
                int minimumScoreNumerator,
                int minimumScoreDenominator,
                int maximumScoreNumerator,
                int maximumScoreDenominator) {
        ...
        public void reduce(Iterable<Tuple3<T, T, IntValue>> values, Collector<Result<T>> out)
            ...
            if (unboundedScores
                    || (count * minimumScoreDenominator >= distinctNeighbors * minimumScoreNumerator
                            && count * maximumScoreDenominator
                                    <= distinctNeighbors * maximumScoreNumerator)) {
     ...
     * @param <T> ID type
     */
    public static class Result<T> extends BinaryResultBase<T>
        ...
        /**
         * Get the Jaccard Index score, equal to the number of shared neighbors of the source and
         * target vertices divided by the number of distinct neighbors.
         *
         * @return Jaccard Index score
         */
        public double getJaccardIndexScore() {
            return getSharedNeighborCount().getValue()
                    / (double) getDistinctNeighborCount().getValue();
        }
