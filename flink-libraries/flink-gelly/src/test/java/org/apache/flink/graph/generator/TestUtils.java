...
public final class TestUtils {
     ...
     * @param expectedEdges edges of the form "source,target" separated by semi-colons; whitespace
     *     is ignored
     ...
     * @throws Exception
     */
    public static <K, VV, EV> void compareGraph(
            Graph<K, VV, EV> graph, String expectedVertices, String expectedEdges)
            throws Exception {
    ...
    private static <K, VV, EV> void compareVertices(Graph<K, VV, EV> graph, String expectedVertices)
        ...
        if (expectedVertices != null) {
            ...
            TestBaseUtils.compareResultAsText(
                    resultVertices, expectedVertices.replaceAll("\\s", "").replace(";", "\n"));
        }
    }
    ...
    private static <K, VV, EV> void compareEdges(Graph<K, VV, EV> graph, String expectedEdges)
        ...
        if (expectedEdges != null) {
            ...
            TestBaseUtils.compareResultAsText(
                    resultEdges, expectedEdges.replaceAll("\\s", "").replace(";", "\n"));
        }
    }
