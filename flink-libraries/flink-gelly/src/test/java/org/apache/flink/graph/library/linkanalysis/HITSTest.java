...
public class HITSTest extends AsmTestBase {
    ...
    public void testWithSimpleGraph() throws Exception {
        DataSet<Result<IntValue>> hits =
                new HITS<IntValue, NullValue, NullValue>(20).run(directedSimpleGraph);
        ...
        for (Result<IntValue> result : hits.collect()) {
            ...
            assertEquals(
                    expectedResults.get(id).f1, result.getAuthorityScore().getValue(), ACCURACY);
        }
    }
    ...
    public void testWithCompleteGraph() throws Exception {
        validate(
                completeGraph, completeGraphVertexCount, 1.0 / Math.sqrt(completeGraphVertexCount));
    }
