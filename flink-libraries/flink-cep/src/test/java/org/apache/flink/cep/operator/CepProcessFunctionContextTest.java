...
public class CepProcessFunctionContextTest extends TestLogger {
    ...
    public void testTimestampPassingForTimedOutInProcessingTime() throws Exception {
        ...
        try (OneInputStreamOperatorTestHarness<Event, String> harness =
            ...
            assertOutput(harness.getOutput()).nextElementEquals("5:A:C").hasNoMoreElements();
    ...
    public void testCurrentProcessingTimeForTimedOutInProcessingTime() throws Exception {
        ...
        try (OneInputStreamOperatorTestHarness<Event, String> harness =
            ...
            assertOutput(harness.getOutput()).nextElementEquals("5:A:B").hasNoMoreElements();
    ...
    /**
     * Creates a {@link PatternProcessFunction} that as a result will produce Strings as follows:
     * {@code [timestamp]:[Event.getName]...} The Event.getName will occur stateNumber times. If the
     * match does not contain n-th pattern it will replace this position with "null".
     *
     * <p>This function will also apply the same logic for timed out partial matches and emit those
     * results into side output described with given output tag.
     *
     * @param stateNumber number of states in the pattern
    ...
    /**
     * Creates a {@link PatternProcessFunction} that as a result will produce Strings as follows:
     * {@code [currentProcessingTime]:[Event.getName]...}. The Event.getName will occur stateNumber
     * times. If the match does not contain n-th pattern it will replace this position with "null".
     *
     * @param stateNumber number of states in the pattern
     * @return created PatternProcessFunction
     */
    private static PatternProcessFunction<Event, String> extractCurrentProcessingTimeAndNames(
            int stateNumber) {
        return new AccessContextWithNames(
                stateNumber, context -> String.valueOf(context.currentProcessingTime()));
    }
