 ...
 * {@link PatternProcessFunction} to be able to handle timed out partial matches implement this
...
public interface TimedOutPartialMatchHandler<IN> {
     ...
     * }</pre>
     *
     * <p>{@link PatternProcessFunction.Context#timestamp()} in this case returns the minimal time
     * in which we can say that the partial match will not become a match, which is effectively the
     * timestamp of the first element assigned to the partial match plus the value of within.
     *
     * @param match map containing the timed out partial match. Events are identified by their
     *     names.
    ...
    void processTimedOutMatch(
            final Map<String, List<IN>> match, final PatternProcessFunction.Context ctx)
            throws Exception;
}
