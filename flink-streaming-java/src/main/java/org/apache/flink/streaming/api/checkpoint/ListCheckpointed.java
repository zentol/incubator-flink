 ...
 *
 * <p>Implementing this interface is a shortcut for obtaining the default {@code ListState} from the
 * {@link OperatorStateStore}. Using the {@code OperatorStateStore} directly gives more flexible
 ...
 *
 * <p>State redistribution happens when the parallelism of the operator is changed. State
 * redistribution of <i>operator state</i> (to which category the state handled by this interface
 * belongs) always goes through a checkpoint, so it appears to the transformation functions like a
 * failure/recovery combination, where recovery happens with a different parallelism.
 *
 * <p>Conceptually, the state in the checkpoint is the concatenated list of all lists returned by
 * the parallel transformation function instances. When restoring from a checkpoint, the list is
 * divided into sub-lists that are assigned to each parallel function instance.
 *
 ...
 * <h2>Example</h2>
 *
 * <p>The following example illustrates how to implement a {@code MapFunction} that counts all
...
public interface ListCheckpointed<T extends Serializable> {
     ...
     *
     * <p>The returned list should contain one entry for redistributable unit of state. See the
     * {@link ListCheckpointed class docs} for an illustration how list-style state redistribution
     * works.
     *
     * <p>As special case, the returned list may be null or empty (if the operator has no state) or
     * it may contain a single element (if the operator state is indivisible).
     *
     ...
     * @return The operator state in a list of redistributable, atomic sub-states. Should not return
     *     null, but empty list instead.
     ...
     *
     * <p>The given state list will contain all the <i>sub states</i> that this parallel instance of
     * the function needs to handle. Refer to the {@link ListCheckpointed class docs} for an
     ...
     *     will re-attempt the recovery, or try recovering from a different checkpoint.
     */
