...
/**
 * Streaming point-to-point latency network benchmarks executed by the external <a
...
public class StreamNetworkPointToPointBenchmark {
     ...
     *
     * @param records records to pass through the network stack
     * @param flushAfterLastEmit whether to flush the {@link RecordWriter} after the last record
     */
    ...
    public void setUp(long flushTimeout, Configuration config) throws Exception {
        ...
        recordWriter =
                new RecordWriterBuilder().setTimeout(flushTimeout).build(resultPartitionWriter);
