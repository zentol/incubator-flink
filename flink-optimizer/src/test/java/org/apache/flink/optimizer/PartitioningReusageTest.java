...
public class PartitioningReusageTest extends CompilerTestBase {
    ...
    private void checkValidJoinInputProperties(DualInputPlanNode join) {
        ...
        if (inProps1.getPartitioning() == PartitioningProperty.HASH_PARTITIONED
            ...
            for (int i = 0; i < pFields1.size(); i++) {
                ...
                if (pos1 < 0) {
                    fail(
                            "Input 1 is partitioned on field "
                                    + f1
                                    + " which is not contained in the key set "
                                    + reqPFields1);
                }
                if (pos2 < 0) {
                    fail(
                            "Input 2 is partitioned on field "
                                    + f2
                                    + " which is not contained in the key set "
                                    + reqPFields2);
                }
            ...
            throw new UnsupportedOperationException(
                    "This method has only been implemented to check for hash partitioned coGroupinputs");
        }
    }
    ...
    private void checkValidCoGroupInputProperties(DualInputPlanNode coGroup) {
        ...
        if (inProps1.getPartitioning() == PartitioningProperty.HASH_PARTITIONED
            ...
            throw new UnsupportedOperationException(
                    "This method has only been implemented to check for hash partitioned coGroup inputs");
        }
    }
