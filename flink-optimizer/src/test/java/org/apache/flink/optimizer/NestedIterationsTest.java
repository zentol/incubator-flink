...
public class NestedIterationsTest extends CompilerTestBase {
    ...
    public void testRejectNestedWorksetIterations() {
        try {
            ...
            DeltaIteration<Tuple2<Long, Long>, Tuple2<Long, Long>> outerIteration =
                    data.iterateDelta(data, 100, 0);

            DataSet<Tuple2<Long, Long>> inOuter =
                    outerIteration.getWorkset().map(new IdentityMapper<Tuple2<Long, Long>>());

            DeltaIteration<Tuple2<Long, Long>, Tuple2<Long, Long>> innerIteration =
                    inOuter.iterateDelta(inOuter, 100, 0);
    ...
    public void testBulkIterationInClosure() {
        try {
            ...
            IterativeDataSet<Long> mainIteration =
                    data2.map(new IdentityMapper<Long>()).iterate(100);
    ...
    public void testDeltaIterationInClosure() {
        try {
            ...
            DeltaIteration<Tuple2<Long, Long>, Tuple2<Long, Long>> firstIteration =
                    data1.iterateDelta(data1, 100, 0);
            ...
            DataSet<Tuple2<Long, Long>> firstResult =
                    firstIteration
                            .closeWith(inFirst, inFirst)
                            .map(new IdentityMapper<Tuple2<Long, Long>>());

            DeltaIteration<Tuple2<Long, Long>, Tuple2<Long, Long>> mainIteration =
                    data2.iterateDelta(data2, 100, 0);
