...
/**
 * Interesting properties are propagated from parent operators to child operators. They tell the
 * child what data properties would help the parent in operating in a cheaper fashion. A reduce
 * operator, for example, tells its child that partitioned data would help. If the child is a join
 * operator, it can use that knowledge to favor strategies that leave the data in a partitioned
 * form.
 *
 * <p>More on optimization with interesting properties can be found in the works on the volcano- and
 * cascades optimizer framework.
 */
public class InterestingProperties implements Cloneable {

    private Set<RequestedGlobalProperties>
            globalProps; // the global properties, i.e. properties across partitions

    private Set<RequestedLocalProperties>
    ...
    public void dropTrivials() {
        for (Iterator<RequestedGlobalProperties> iter = this.globalProps.iterator();
                iter.hasNext(); ) {
        ...
        for (Iterator<RequestedLocalProperties> iter = this.localProps.iterator();
                iter.hasNext(); ) {
    ...
    public String toString() {
        return "InterestingProperties [globalProps="
                + this.globalProps
                + ", localProps="
                + this.localProps
                + " ]";
