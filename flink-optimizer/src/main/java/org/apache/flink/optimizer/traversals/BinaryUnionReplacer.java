    ...
    public void postVisit(PlanNode visitable) {
        ...
        if (visitable instanceof BinaryUnionPlanNode) {
            ...
            if (!unionNode.unionsStaticAndDynamicPath()) {
                ...
                newUnionNode =
                        new NAryUnionPlanNode(
                                unionNode.getOptimizerNode(),
                                inputs,
                                unionNode.getGlobalProperties(),
                                unionNode.getCumulativeCosts());
                ...
                // make sure that the first input is the cached (static) and the second input is the
                // dynamic
    ...
    public void collect(Channel in, List<Channel> inputs) {
        if (in.getSource() instanceof NAryUnionPlanNode) {
            ...
            if (!(in.getLocalStrategy() == null || in.getLocalStrategy() == LocalStrategy.NONE)) {
                throw new CompilerException(
                        "Bug: Plan generation for Unions picked a local strategy between binary plan operators.");
            }
