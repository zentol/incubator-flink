    ...
    private int translateChannel(
        ...
        if (inputPlanNode instanceof NAryUnionPlanNode) {
            ...
            if (input.getShipStrategy() != ShipStrategyType.FORWARD && !isBroadcast) {
                throw new CompilerException(
                        "Optimized plan contains Union with non-forward outgoing ship strategy.");
            }
            ...
            // for now, skip connections with the solution set node, as this is a local index access
            // (later to be parameterized here)
        ...
        // expand the channel to all the union channels, in case there is a union operator at its
        // source
    ...
    private void finalizeBulkIteration(IterationDescriptor descr) {
        ...
        if (rootOfTerminationCriterion != null
            ...
            JobVertex rootOfTerminationCriterionVertex =
                    this.vertices.get(rootOfTerminationCriterion);
            ...
            if (rootOfTerminationCriterionVertex == null) {
                ...
                if (taskInChain == null) {
                    throw new CompilerException(
                            "Bug: Tail of termination criterion not found as vertex or chained task.");
                }
            ...
            tailConfigOfTerminationCriterion.setOutputSerializer(
                    bulkNode.getSerializerForIterationChannel());
...

        {
...

            {
                ...
                if (nextWorksetVertex == null) {
                    ...
                    if (taskInChain == null) {
                        throw new CompilerException(
                                "Bug: Next workset node not found as vertex or chained task.");
                    }
            ...
            }
            {
                ...
                if (solutionDeltaVertex == null) {
                    ...
                    if (taskInChain == null) {
                        throw new CompilerException(
                                "Bug: Solution Set Delta not found as vertex or chained task.");
                    }
    ...
    private void attachOperatorNamesAndDescriptions() {
        ...
        for (int i = chainedTasksInSequence.size() - 1; i >= 0; i--) {
            ...
            String opDescription =
                    JsonMapper.getOperatorStrategyString(planNode.getDriverStrategy());
            if (vertex.getOperatorDescription() == null) {
                vertex.setOperatorDescription(opDescription);
            } else {
                vertex.setOperatorDescription(
        ...
        for (Map.Entry<PlanNode, JobVertex> entry : vertices.entrySet()) {
            ...
            // operator description
            String opStrategy =
                    JsonMapper.getOperatorStrategyString(
