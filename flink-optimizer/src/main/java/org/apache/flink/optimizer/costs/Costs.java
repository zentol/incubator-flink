...
/**
 * Simple class to represent the costs of an operation. The costs are currently tracking, network,
 * I/O and CPU costs.
 *
 * <p>Costs are composed of two parts of cost contributors:
 *
 * <ol>
 *   <li>Quantifiable costs. Those costs are used when estimates are available and track a
 *       quantifiable measure, such as the number of bytes for network or I/O
 *   <li>Heuristic costs. Those costs are used when no estimates are available. They can be used to
 *       track that an operator used a special operation which is heuristically considered more
 *       expensive than another operation.
 * </ol>
 *
 * <p>The quantifiable costs may frequently be unknown, which is represented by a {@code -1} as a
