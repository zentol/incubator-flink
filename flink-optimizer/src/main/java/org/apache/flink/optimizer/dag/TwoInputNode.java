 ...
 * Join, Cross, CoGroup, or Union. The two inputs are not substitutable in their sides.
 */
public abstract class TwoInputNode extends OptimizerNode {
    ...
    public void setInput(
            Map<Operator<?>, OptimizerNode> contractToNode, ExecutionMode defaultExecutionMode) {
        ...
        if (shipStrategy != null) {
            if (Optimizer.HINT_SHIP_STRATEGY_FORWARD.equals(shipStrategy)) {
                ...
                throw new CompilerException(
                        "Unknown hint for shipping strategy of input one: " + shipStrategy);
            }
        }
        ...
        if (shipStrategy != null) {
            if (Optimizer.HINT_SHIP_STRATEGY_FORWARD.equals(shipStrategy)) {
                ...
                throw new CompilerException(
                        "Unknown hint for shipping strategy of input two: " + shipStrategy);
            }
        }
        ...
        if (leftPred == null) {
            throw new CompilerException(
                    "Error: Node for '"
                            + getOperator().getName()
                            + "' has no input set for first input.");
        } else {
        ...
        if (rightPred == null) {
            throw new CompilerException(
                    "Error: Node for '"
                            + getOperator().getName()
                            + "' has no input set for second input.");
        } else {
            ...
            for (PlanNode child2 : subPlans2) {
                ...
                for (RequestedGlobalProperties igps1 : intGlobal1) {
                    ...
                    for (RequestedGlobalProperties igps2 : intGlobal2) {
                        ...
                        // connection says so
                        final Channel c2 =
                                new Channel(child2, this.input2.getMaterializationMode());
                        if (this.input2.getShipStrategy() == null) {
                            // free to choose the ship strategy
                            igps2.parameterizeChannel(
                                    c2, dopChange2, input2Mode, input2breaksPipeline);

                            // if the parallelism changed, make sure that we cancel out properties,
                            // unless the
                            // ship strategy preserves/establishes them even under changing
                            // parallelisms
                            ...
                            DataExchangeMode exMode =
                                    DataExchangeMode.select(
                                            input2Mode, shipType, input2breaksPipeline);
                            if (this.keys2 != null) {
                        ...
                        for (GlobalPropertiesPair gpp : allGlobalPairs) {
                            if (gpp.getProperties1().isMetBy(c1.getGlobalProperties())
                                ...
                                for (OperatorDescriptorDual desc : getProperties()) {
                                    if (desc.areCompatible(
                                            gpp.getProperties1(),
                                            gpp.getProperties2(),
                                            c1.getGlobalProperties(),
                                            c2.getGlobalProperties())) {
                        ...
                        // is fixed via a hint. All the properties are overridden by the hint
                        // anyways,
    ...
    protected void addLocalCandidates(
            ...
            LocalPropertiesPair[] validLocalCombinations,
                ...
                for (OperatorDescriptorDual dps : getProperties()) {
                    for (LocalPropertiesPair lpp : dps.getPossibleLocalProperties()) {
                        if (lpp.getProperties1().isMetBy(in1.getLocalProperties())
                            ...
                            // valid combination
                            // for non trivial local properties, we need to check that they are co
                            // compatible
                            // (such as when some sort order is requested, that both are the same
                            // sort order
                            if (dps.areCoFulfilled(
                                    lpp.getProperties1(),
                                    lpp.getProperties2(),
                                    in1.getLocalProperties(),
    ...
    protected void instantiate(
        ...
        for (List<NamedChannel> broadcastChannelsCombination :
            ...
            for (int i = 0; i < broadcastChannelsCombination.size(); i++) {
                ...
                if (!(areBranchCompatible(bcSource, inputSource1)
                        || areBranchCompatible(bcSource, inputSource2))) {
            ...
            SemanticProperties semPropsLocalPropFiltering =
    ...
    protected void placePipelineBreakersIfNecessary(
            DriverStrategy strategy, Channel in1, Channel in2) {
        // before we instantiate, check for deadlocks by tracing back to the open branches and
        // checking
        // whether either no input, or all of them have a dam
        if (in1.isOnDynamicPath()
                && in2.isOnDynamicPath()
                && this.hereJoinedBranches != null
            ...
            if (strategy.firstDam() == DamBehavior.FULL_DAM
                    || in1.getLocalStrategy().dams()
                    || in1.getTempMode().breaksPipeline()) {
                someDamOnLeftPaths = true;
            } else {
                for (OptimizerNode brancher : this.hereJoinedBranches) {
                    PlanNode candAtBrancher = in1.getSource().getCandidateAtBranchPoint(brancher);

                    // not all candidates are found, because this list includes joined branched from
                    // both regular inputs and broadcast vars
            ...
            if (strategy.secondDam() == DamBehavior.FULL_DAM
                    || in2.getLocalStrategy().dams()
                    || in2.getTempMode().breaksPipeline()) {
    ...
    public void computeUnclosedBranchStack() {
        ...
        ArrayList<UnclosedBranchDescriptor> inputsMerged =
