...
/** Specialized {@link BulkWriter} which is expected to write to specified {@link Path}. */
@Internal
public interface HadoopPathBasedBulkWriter<T> extends BulkWriter<T> {
    ...
    /**
     * Disposes the writer on failures. Unlike output-stream-based writers which could handled
     * uniformly by closing the underlying output stream, the path- based writers need to be
     * disposed explicitly.
     */
    void dispose();
    ...
    interface Factory<T> extends Serializable {
        ...
        /**
         * Creates a path-based writer that writes to the <tt>inProgressPath</tt> first and commits
         * to <tt>targetPath</tt> finally.
         *
