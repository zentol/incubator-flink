 ...
 *
 * <p>Records are emitted by selecting the oldest available element of all producer queues, as long
 * as the timestamp does not exceed the current shared watermark plus allowed lookahead interval.
 *
 * @param <T>
 */
@Internal
public abstract class RecordEmitter<T extends TimestampedValue> implements Runnable {
     ...
     *
     * <p>Larger queue size can lead to higher throughput, but also to very high heap space
     * consumption, depending on the size of elements.
     *
     * <p>Note that this is difficult to tune, because it does not take into account the size of
     * individual objects.
     */
    ...
    private final ConcurrentHashMap<AsyncRecordQueue<T>, Boolean> emptyQueues =
            new ConcurrentHashMap<>();
    private final PriorityQueue<AsyncRecordQueue<T>> heads =
            new PriorityQueue<>(this::compareHeadElement);
