 ...
 * /{basePath}/{dateTimePath}/}. The {@code basePath} is the path that was specified as a base path
 * when creating the new bucket. The {@code dateTimePath} is determined based on the current system
 * time and the user provided format string.
 *
 * <p>{@link DateTimeFormatter} is used to derive a date string from the current system time and the
 * date format string. The default format string is {@code "yyyy-MM-dd--HH"} so the rolling files
 ...
 * }</pre>
 *
 * <p>This will create for example the following bucket path: {@code /base/1976-12-31-14/}
 */
@PublicEvolving
public class DateTimeBucketAssigner<IN> implements BucketAssigner<IN, String> {
     ...
     *
     * @param formatString The format string that will be given to {@code SimpleDateFormat} to
    ...
    /**
     * Creates a new {@code DateTimeBucketAssigner} with the given date/time format string using the
     * given timezone.
     *
