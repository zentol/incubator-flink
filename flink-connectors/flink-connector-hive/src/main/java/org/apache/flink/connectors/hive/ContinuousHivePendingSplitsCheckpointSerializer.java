...
public class ContinuousHivePendingSplitsCheckpointSerializer
    ...
    private final ListSerializer<String> partitionSerDe =
            new ListSerializer<>(StringSerializer.INSTANCE);
    // SerDe for the current read offset
    private final ReadOffsetSerDe readOffsetSerDe = ReadOffsetSerDeImpl.INSTANCE;

    public ContinuousHivePendingSplitsCheckpointSerializer(
            SimpleVersionedSerializer<HiveSourceSplit> splitSerDe) {
    ...
    @Override
    public byte[] serialize(PendingSplitsCheckpoint<HiveSourceSplit> checkpoint)
            throws IOException {
        Preconditions.checkArgument(
                checkpoint.getClass() == ContinuousHivePendingSplitsCheckpoint.class,
                "Only supports %s",
    ...
    @Override
    public PendingSplitsCheckpoint<HiveSourceSplit> deserialize(int version, byte[] serialized)
            throws IOException {
        if (version == 1) {
    ...
    private PendingSplitsCheckpoint<HiveSourceSplit> deserializeV1(
    ...
    interface ReadOffsetSerDe {
        ...
        Comparable<?> deserialize(InputStream inputStream)
                throws IOException, ClassNotFoundException;
    }
}
